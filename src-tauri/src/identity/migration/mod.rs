...
use self::report::{MigrationReport, FileStatus};
...
impl MigrationManager {
    ...
    async fn process_file(&mut self, path: &Path, report: &mut MigrationReport) -> Result<FileStatus> {
        ...
        let legacy_ids = if self.config.include_legacy_ids {
            Some(LegacyIdMapper::calculate_legacy_ids(path, &self.vault_root)?)
        } else {
            None
        };
        ...
        if self.config.dry_run {
            report.dry_run_changes.push(format!(
                "Would add UUID to: {}{}",
                path.display(),
                if legacy_ids.is_some() { " (with legacy ID mapping)" } else { "" }
            ));
    ...
    async fn store_legacy_ids(&self, path: &Path, uuid: &str, legacy_ids: &mapper::LegacyIds) -> Result<()> {
        ...
        if SidecarManager::should_use_sidecar(path) {
            ...
            let mut data = SidecarManager::read(path)?
                .unwrap_or_else(|| SidecarData::new(uuid.to_string(), path.to_string_lossy().to_string()));
            ...
            data.file_hash = Some(format!("{}|{}", 
                legacy_ids.original_absolute_path, 
                legacy_ids.original_relative_path));
            
            SidecarManager::write(path, &data)?;
        } else if path.exists() {
            // For markdown files, update front matter
            let content = std::fs::read_to_string(path)?;
            let (mut fm, body) = FrontMatterParser::parse(&content)?;
            ...
            front_matter.other.insert("original_absolute_path".to_string(), yaml_rust::Yaml::String(legacy_ids.original_absolute_path.clone()));
            front_matter.other.insert("original_relative_path".to_string(), yaml_rust::Yaml::String(legacy_ids.original_relative_path.clone()));
