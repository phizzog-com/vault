...
use notify_debouncer_full::{
    new_debouncer,
    notify::{EventKind, RecursiveMode, Watcher, Event, event::{ModifyKind, RenameMode}},
...
impl IdentityWatcher {
    pub fn new(
        ...
        Self {
            ...
            deletion_cache: Arc::new(RwLock::new(
                DeletionCache::new(config.cache_size, config.rename_window_ms)
            )),
            config,
            vault_root,
        }
    ...
    async fn handle_creation(&mut self, event: &DebouncedEvent) -> Result<()> {
        if let Some(path) = event.paths.first() {
            ...
            if let Some(old_metadata) = possible_rename {
                if self.config.debug {
                    println!("Possible rename detected: {:?} -> {:?}", 
