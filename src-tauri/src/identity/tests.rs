...
fn test_concurrent_id_generation() {
    let temp_dir = TempDir::new().unwrap();
    let manager = Arc::new(RwLock::new(IdentityManager::new(temp_dir.path().to_path_buf())));
...
fn test_path_with_special_characters() {
    ...
    let special_file = temp_dir.path().join("test file with spaces & special@chars!.md");
