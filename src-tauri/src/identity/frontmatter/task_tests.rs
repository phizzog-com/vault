...
This is the main content."#;
    ...
    fn test_write_frontmatter_with_tasks() {
        ...
        task1.insert("status".to_string(), serde_json::Value::String("todo".to_string()));
        task1.insert("text".to_string(), serde_json::Value::String("Complete feature".to_string()));
        task1.insert("priority".to_string(), serde_json::Value::String("high".to_string()));
        task1.insert("created_at".to_string(), serde_json::Value::String("2025-08-24T10:00:00Z".to_string()));
        task1.insert("updated_at".to_string(), serde_json::Value::String("2025-08-24T10:00:00Z".to_string()));
        
        tasks.insert("task-id-1".to_string(), serde_json::Value::Object(task1));
        
        fm.extra_fields.insert("tasks".to_string(), serde_json::Value::Object(tasks));
...
Content"#;
        ...
        if let Some(serde_json::Value::Object(ref mut tasks)) = fm.extra_fields.get_mut("tasks") {
            if let Some(serde_json::Value::Object(ref mut task)) = tasks.get_mut("task-uuid-1") {
                task.insert("status".to_string(), serde_json::Value::String("done".to_string()));
                task.insert("priority".to_string(), serde_json::Value::String("high".to_string()));
                task.insert("completed_at".to_string(), 
...
---"#;
        ...
        if let Some(serde_json::Value::Object(ref tasks)) = merged.extra_fields.get("tasks") {
            if let Some(serde_json::Value::Object(ref task)) = tasks.get("task-1") {
                assert_eq!(task.get("status"), Some(&serde_json::Value::String("done".to_string())));
                assert_eq!(task.get("priority"), Some(&serde_json::Value::String("high".to_string())));
            }
        }
    }
...
Content"#;
    ...
    fn merge_task_updates(
        ...
        if let Some(serde_json::Value::Object(ref tasks2)) = update2.extra_fields.get("tasks") {
            let tasks_mut = merged.extra_fields.entry("tasks".to_string())
            ...
            if let serde_json::Value::Object(ref mut tasks1) = tasks_mut {
                for (task_id, task2) in tasks2 {
                    if let Some(task2_obj) = task2.as_object() {
                        let task1_mut = tasks1.entry(task_id.clone())
                            .or_insert(serde_json::Value::Object(serde_json::Map::new()));
    ...
    fn batch_update_tasks(
        ...
        let tasks_mut = fm.extra_fields.entry("tasks".to_string())
            .or_insert(serde_json::Value::Object(serde_json::Map::new()));
        ...
        if let serde_json::Value::Object(ref mut tasks) = tasks_mut {
            for (task_id, props) in updates {
                ...
                task_obj.insert("created_at".to_string(), 
                               serde_json::Value::String(Utc::now().to_rfc3339()));
