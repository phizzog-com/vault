...
impl TaskFrontMatter {
    ...
    pub fn extract_tasks(extra_fields: &std::collections::BTreeMap<String, serde_json::Value>) 
    ...
    fn parse_task_properties(obj: &serde_json::Map<String, serde_json::Value>) 
        -> Result<TaskProperties> 
    {
        ...
        let completed_at = obj.get("completed_at")
            .and_then(|v| v.as_str())
            .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
            .map(|dt| dt.with_timezone(&Utc));
    ...
    pub fn task_to_json(task: &TaskProperties) -> serde_json::Value {
        ...
        obj.insert("status".to_string(), 
                   serde_json::Value::String(match task.status {
                       TaskStatus::Todo => "todo",
                       TaskStatus::Done => "done",
                   }.to_string()));
        
        obj.insert("text".to_string(), serde_json::Value::String(task.text.clone()));
        
        if let Some(ref due) = task.due {
            obj.insert("due".to_string(), 
                      serde_json::Value::String(due.to_rfc3339()));
        }
        
        if let Some(ref priority) = task.priority {
            obj.insert("priority".to_string(), 
                      serde_json::Value::String(match priority {
                          Priority::High => "high",
        ...
        if let Some(ref tags) = task.tags {
            let tags_array: Vec<serde_json::Value> = tags.iter()
        ...
        if let Some(ref project) = task.project {
            obj.insert("project".to_string(), serde_json::Value::String(project.clone()));
        }
        ...
        if let Some(ref completed) = task.completed_at {
            obj.insert("completed_at".to_string(), 
    ...
    pub fn remove_task_atomic(
        file_path: &Path,
        task_id: &str
    ) -> Result<Option<TaskProperties>> {
        // Read current file
        let content = std::fs::read_to_string(file_path)?;
        let (fm, body) = FrontMatterParser::parse(&content)?;
        let mut fm = fm.ok_or_else(|| anyhow::anyhow!("No front matter found"))?;
