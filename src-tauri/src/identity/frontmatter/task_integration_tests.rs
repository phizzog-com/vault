...
mod tests {
    ...
    fn test_due_date_normalization() {
        ...
        assert_eq!(normalized.date_naive(), 
    ...
    fn test_merge_concurrent_task_updates() {
        let original_tasks = {
            ...
            tasks.insert("task-1".to_string(), TaskProperties::new("Task 1".to_string()));
            tasks.insert("task-2".to_string(), TaskProperties::new("Task 2".to_string()));
            tasks
        };
        ...
        let update2_tasks = {
            ...
            tasks.insert("task-3".to_string(), TaskProperties::new("Task 3".to_string()));
            tasks
        };
    ...
    fn test_frontmatter_field_ordering() {
        ...
        fm.extra_fields.insert("author".to_string(), 
            serde_json::Value::String("John Doe".to_string()));
        fm.extra_fields.insert("category".to_string(), 
        ...
        let created_idx = lines.iter().position(|l| l.starts_with("created_at:")).unwrap();
        let updated_idx = lines.iter().position(|l| l.starts_with("updated_at:")).unwrap();
...
Content"#;
    ...
    fn test_batch_task_operations() {
        ...
        assert_eq!(extracted.get("task-0").unwrap().priority, Some(Priority::High));
