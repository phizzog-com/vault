...
use crate::identity::frontmatter::{FrontMatter, FrontMatterParser, FrontMatterWriter, TaskFrontMatter, TaskProperties, TaskStatus as FmTaskStatus, Priority};
use super::parser::{TaskParser, ParsedTask, TaskStatus};
...
impl TaskIdentity {
    ...
    pub fn update_task_with_frontmatter(&self, file_path: &Path, line_number: usize, task_id: &str, parsed_task: &ParsedTask) -> Result<()> {
        let content = fs::read_to_string(file_path)?;
        
        // Parse front matter
        let (mut front_matter, body_content) = FrontMatterParser::parse(&content)?;
        ...
        if let Some(priority) = parsed_task.properties.get("priority") {
            task_props.priority = Some(crate::identity::frontmatter::tasks::normalize_priority(priority));
        }
    ...
    pub fn update_task_line(&self, file_path: &Path, line_number: usize, task_id: &str) -> Result<()> {
        let content = fs::read_to_string(file_path)?;
    ...
    fn batch_update_file(&self, file_path: &Path, updates: HashMap<usize, String>) -> Result<()> {
        ...
        for (line_number, task_id) in updates {
            if line_number > 0 && line_number <= lines.len() {
                lines[line_number - 1] = TaskParser::add_tid_to_line(&lines[line_number - 1], &task_id);
            }
        }
    ...
    pub fn find_duplicate_task_ids(&self, vault_root: &Path) -> Result<HashMap<String, Vec<PathBuf>>> {
