...
mod tests {
    ...
    fn test_parse_task_with_existing_id() {
        ...
        assert_eq!(task.id, Some("01234567-89ab-cdef-0123-456789abcdef".to_string()));
    }
    ...
    fn test_parse_task_with_project_colon_syntax() {
        ...
        assert!(result.is_some());
        let task = result.unwrap();
        assert_eq!(task.properties.get("project"), Some(&"enterprise".to_string()));
    }
    ...
    fn test_parse_task_with_project() {
        ...
        assert!(result.is_some());
        let task = result.unwrap();
        assert_eq!(task.properties.get("project"), Some(&"ProductLaunch".to_string()));
    }
    ...
    fn test_parse_task_with_all_properties() {
        ...
        assert_eq!(task.properties.get("project"), Some(&"Sprint23".to_string()));
    ...
    fn test_parse_task_with_special_characters() {
        ...
        assert!(result.is_some());
        let task = result.unwrap();
        assert_eq!(task.content, "Fix bug in `parse_line()` function & test $variables");
    }
