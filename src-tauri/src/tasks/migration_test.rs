...
mod task_migration_tests {
    ...
    use crate::tasks::migration::{TaskMigrationManager, TaskMigrationConfig, TaskMigrationReport, TaskFileStatus};
    use std::sync::Arc;
    ...
    async fn test_task_migration_idempotency() {
        ...
        let mut manager2 = TaskMigrationManager::new(
            identity_manager.clone(),
            vault_path.clone(),
            config,
        );
    ...
    async fn test_parallel_processing() {
        ...
        assert!(duration.as_secs() < 5, "Migration took too long: {:?}", duration);
    }
