...
mod tests {
    ...
    impl TestEnv {
        fn new() -> Self {
            ...
            let identity_manager = Arc::new(Mutex::new(
                IdentityManager::new(temp_dir.path().to_path_buf())
            ));
    ...
    async fn test_ensure_task_uuid_creates_new_uuid() {
        ...
        assert!(result.is_ok(), "Failed with error: {:?} for file: {:?}", result.err(), file_path);
...
"#,
        ...
        assert_eq!(fm_task.status, crate::identity::frontmatter::tasks::TaskStatus::Todo);
...
"#,
        ...
        let task_line_index = lines.iter().position(|l| l.contains("- [ ] Test task")).unwrap();
        let line = lines[task_line_index];
        let toggled_line = crate::identity::tasks::TaskParser::toggle_task_status(line);
        
        assert!(toggled_line.contains("[x]"), "Toggled line should contain [x], got: {}", toggled_line);
        
        // Verify we can update front matter
        let (front_matter, _) = FrontMatterParser::parse(&content).unwrap();
        let mut fm = front_matter.unwrap();
        let mut tasks = crate::identity::frontmatter::TaskFrontMatter::extract_tasks(&fm.extra_fields).unwrap();
        
        if let Some(task) = tasks.get_mut(task_id) {
            task.mark_done();
        }
        
        assert_eq!(tasks.get(task_id).unwrap().status, crate::identity::frontmatter::tasks::TaskStatus::Done);
...
"#,
        ...
        if let Some(task) = tasks.get_mut(task_id) {
            ...
            task.priority = Some(crate::identity::frontmatter::tasks::normalize_priority("high"));
        ...
        // Write back
        crate::identity::frontmatter::FrontMatterWriter::write_atomic(&file_path, &fm, &body).unwrap();
        
        // Verify update
        let updated_content = env.read_file(&file_path);
        let (updated_fm, _) = FrontMatterParser::parse(&updated_content).unwrap();
        let updated_tasks = crate::identity::frontmatter::TaskFrontMatter::extract_tasks(&updated_fm.unwrap().extra_fields).unwrap();
        let task = updated_tasks.get(task_id).unwrap();
        
        assert_eq!(task.status, crate::identity::frontmatter::tasks::TaskStatus::Done);
        assert_eq!(task.text, "Updated task text");
        assert!(task.due.is_some());
        assert_eq!(task.priority, Some(crate::identity::frontmatter::Priority::High));
...
"#;
    ...
    async fn test_performance_ensure_task_uuid() {
        ...
        assert!(result.is_ok());
        assert!(duration.as_millis() < 50, "Operation took {}ms, expected <50ms", duration.as_millis());
    }
