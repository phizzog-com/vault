...
impl PtyManager {
    ...
    pub async fn spawn_pty(
        ...
        println!("PTY: Creating pty with size {}x{}", options.rows, options.cols);
        ...
        println!("PTY: Building command: {} {:?}", options.command, options.args);
        ...
        let mut child = pty_pair
            ...
            .map_err(|e| {
                ...
                format!("Failed to spawn command '{}': {}. PATH: {}", options.command, e, full_path)
            })?;
            ...
            loop {
                match reader.read(&mut buffer) {
                    ...
                    Err(e) => {
                        ...
                        let _ = app_handle_clone.emit(&format!("pty:error:{}", session_id_clone), &e.to_string());
                        break;
                    }
                }
            }
