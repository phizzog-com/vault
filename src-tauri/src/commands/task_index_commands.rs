...
use crate::tasks::{TaskRecord, IndexStats, TaskQuery};
...
pub async fn get_task_source_by_id(
    ...
    let record = index.get_task(&task_id).await
...
pub async fn sync_file_tasks_to_index(
    ...
    eprintln!("[sync_file_tasks_to_index command] Path exists: {}", path.exists());
    eprintln!("[sync_file_tasks_to_index command] Path is absolute: {}", path.is_absolute());
...
pub async fn verify_task_index_consistency(
    ...
    index.verify_consistency().await
...
mod tests {
    ...
    fn test_task_query_request_deserialization() {
        ...
        assert_eq!(query.tags, Some(vec!["urgent".to_string(), "review".to_string()]));
    }
}
