...
use crate::identity::tasks::{ParsedTask, TaskStatus};
use crate::identity::frontmatter::{TaskProperties, Priority};
...
pub async fn ensure_task_uuid(
    ...
    if let Some(content) = line_content {
        ...
        if let Some(task) = crate::identity::tasks::TaskParser::parse_line(&content, line_number) {
            ...
            let new_uuid = manager.generate_task_id()
    ...
    let mut manager = identity_manager.lock().await;
    let uuid = manager.ensure_task_id(Path::new(&file_path), line_number)
        ...
        let (created_at, updated_at, completed_at, due, priority, tags, project) = 
            if let Some(fm_task) = fm_tasks.get(&task_id) {
                (
                    ...
                    fm_task.priority.as_ref().map(|p| format!("{:?}", p).to_lowercase()),
                    fm_task.tags.clone(),
...
pub async fn toggle_task_by_id(
    ...
    let new_status = if toggled_line.contains("[x]") { "done" } else { "todo" };
    ...
    let (mut front_matter, body_only) = crate::identity::frontmatter::FrontMatterParser::parse(&content)
        .map_err(|e| format!("Failed to parse front matter: {}", e))?;
    ...
    let final_content = if let Some(ref task) = task_found {
        ...
        if let Some(fm_task) = tasks.get_mut(&task_id) {
            ...
            let mut new_task = crate::identity::frontmatter::TaskProperties::new(task.content.clone());
    ...
    let manager = identity_manager.lock().await;
    manager.sync_file_tasks_to_index_async(path).await
...
pub async fn toggle_task_status(
    ...
    let final_content = if let Some(task_id) = task.id {
        ...
        let (mut front_matter, body) = crate::identity::frontmatter::FrontMatterParser::parse(&content)
            .map_err(|e| format!("Failed to parse front matter: {}", e))?;
        ...
        if let Some(fm_task) = tasks.get_mut(&task_id) {
            ...
            if let Some(priority) = task.properties.get("priority") {
                new_task.priority = Some(crate::identity::frontmatter::tasks::normalize_priority(priority));
            }
    ...
    let manager = identity_manager.lock().await;
    manager.sync_file_tasks_to_index(path)
        .map_err(|e| format!("Failed to sync task to index: {}", e))?;
...
pub async fn update_task_properties(
    ...
    let (mut fm, body) = crate::identity::frontmatter::FrontMatterParser::parse(&content)
        .map_err(|e| format!("Failed to parse front matter: {}", e))?;
        ...
        match manager_lock.get_task_by_id(path, &request.task_id) {
            Ok(Some(parsed)) => {
                ...
                if let Some(due) = parsed.properties.get("due") {
                    if let Ok(dt) = crate::identity::frontmatter::tasks::normalize_due_date(due) { new_task.due = Some(dt); }
                }
                if let Some(priority) = parsed.properties.get("priority") {
                    new_task.priority = Some(crate::identity::frontmatter::tasks::normalize_priority(priority));
                }
                if let Some(tags) = parsed.properties.get("tags") { new_task.tags = Some(tags.split(',').map(|s| s.trim().to_string()).collect()); }
                if let Some(project) = parsed.properties.get("project") { new_task.project = Some(project.clone()); }
    ...
    if let Some(task) = tasks.get_mut(&request.task_id) {
        ...
        if let Some(text) = request.updates.text { task.text = text; task.updated_at = chrono::Utc::now(); }
        if let Some(due) = request.updates.due { task.due = crate::identity::frontmatter::tasks::normalize_due_date(&due).ok(); task.updated_at = chrono::Utc::now(); }
        if let Some(priority) = request.updates.priority { task.priority = Some(crate::identity::frontmatter::tasks::normalize_priority(&priority)); task.updated_at = chrono::Utc::now(); }
        if let Some(tags) = request.updates.tags { task.tags = Some(tags); task.updated_at = chrono::Utc::now(); }
        if let Some(project) = request.updates.project { task.project = Some(project); task.updated_at = chrono::Utc::now(); }
    }
...
pub async fn batch_ensure_task_uuids(
    ...
    let uuids = manager.batch_ensure_task_ids(path)
...
pub async fn rollback_task_migration(
    ...
    // Perform rollback
    migration_manager.rollback(&report).await
...
pub async fn open_file_at_line(
    ...
    window.emit("open-file-at-line", serde_json::json!({
        "filePath": file_path,
        "lineNumber": line_number
    }))
