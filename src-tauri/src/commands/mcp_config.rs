...
pub async fn validate_mcp_server(
    ...
    if !valid_types.contains(&config.server_type.as_str()) {
        errors.push(format!("Invalid server type for {}: {}", name, config.server_type));
    }
    ...
    if config.command.chars().any(|c| shell_metacharacters.contains(&c)) {
...
"#.to_string();
    ...
    async fn test_validate_mcp_server_rejects_invalid_type() {
        ...
        let validation = result.unwrap();
        assert!(!validation.valid);
        assert!(validation.errors.iter().any(|e| e.contains("Invalid server type")));
    }
    ...
    async fn test_validate_mcp_server_detects_shell_metacharacters_in_command() {
        ...
        let validation = result.unwrap();
        assert!(!validation.valid);
        assert!(validation.errors.iter().any(|e| e.contains("Invalid characters in command")));
    }
    ...
    async fn test_validate_mcp_server_detects_shell_metacharacters_in_args() {
        ...
        let validation = result.unwrap();
        assert!(!validation.valid);
        assert!(validation.errors.iter().any(|e| e.contains("Invalid characters in args")));
    }
