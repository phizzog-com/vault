...
impl GhosttyManager {
    ...
    pub async fn spawn_process(&self, options: GhosttySpawnOptions, app_handle: AppHandle) -> Result<bool, String> {
        ...
        let binary_path = self.detect_ghostty_binary().await
        ...
        match cmd.spawn() {
            Ok(child) => {
                ...
                // Emit spawn event
                let _ = app_handle.emit("ghostty:spawned", serde_json::json!({
                    "success": true,
                    "pid": pid
                }));
            ...
            Err(e) => {
                Err(format!("Failed to spawn Ghostty: {}", e))
            }
        }
    }
