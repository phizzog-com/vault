...
#[tauri::command]
pub async fn check_mcp_servers_status(
    ...
    let servers_path = if mcp_servers_dir.exists() {
        println!("🔧 Using production MCP servers path (_up_): {:?}", mcp_servers_dir);
        mcp_servers_dir
    } else if mcp_servers_alt.exists() {
        println!("🔧 Using production MCP servers path: {:?}", mcp_servers_alt);
        mcp_servers_alt
    } else {
    ...
    for entry in entries.flatten() {
        if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
            ...
            status.insert(server_name, json!({
...
pub async fn setup_mcp_servers(
    ...
    let servers_path = if mcp_servers_dir.exists() {
        println!("🚀 Setting up MCP servers in production at (_up_): {:?}", mcp_servers_dir);
        mcp_servers_dir
    } else if mcp_servers_alt.exists() {
        println!("🚀 Setting up MCP servers in production at: {:?}", mcp_servers_alt);
        mcp_servers_alt
    } else {
        // Development - use parent directory
        if let Ok(current_dir) = std::env::current_dir() {
            if current_dir.file_name().and_then(|s| s.to_str()) == Some("src-tauri") {
                if let Some(parent) = current_dir.parent() {
                    let dev_path = parent.join("mcp-servers");
                    println!("🚀 Setting up MCP servers in development at: {:?}", dev_path);
                    dev_path
                } else {
