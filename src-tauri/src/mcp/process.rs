            ...
            if cfg!(debug_assertions) {
                ...
                if let Ok(current_dir) = std::env::current_dir() {
                    ...
                    let possible_paths = vec![
                        ...
                        current_dir.parent().map(|p| p.join(format!("mcp-servers/{}/target/debug/{}", project_dir, command))),
                        // Debug builds - from project root
                        Some(current_dir.join(format!("mcp-servers/{}/target/debug/{}", project_dir, command))),
