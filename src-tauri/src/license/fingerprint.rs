...
mod tests {
    ...
    fn test_machine_fingerprint_consistent() {
        ...
        let fingerprint1 = get_machine_fingerprint()
            .expect("Failed to get machine fingerprint");
        let fingerprint2 = get_machine_fingerprint()
            .expect("Failed to get machine fingerprint");
        let fingerprint3 = get_machine_fingerprint()
            .expect("Failed to get machine fingerprint");

        assert_eq!(fingerprint1, fingerprint2, "Fingerprint should be consistent");
        assert_eq!(fingerprint2, fingerprint3, "Fingerprint should be consistent");
    }
    ...
    fn test_fingerprint_not_empty() {
        ...
        assert!(fingerprint.len() > 0, "Fingerprint should have positive length");
    }
}
