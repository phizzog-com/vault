...
pub async fn load_schema(csv_path: &Path) -> Result<CsvSchema, CsvError> {
    ...
    let content = fs::read_to_string(&path).await.map_err(|e| CsvError::ReadError {
        message: format!("Failed to read schema file: {}", e),
    })?;

    let schema: CsvSchema = serde_json::from_str(&content).map_err(|e| {
        CsvError::SchemaParseError {
...
pub async fn save_schema(csv_path: &Path, schema: &CsvSchema) -> Result<(), CsvError> {
    ...
    // Write to temp file
    let mut file = fs::File::create(&temp_path).await.map_err(|e| CsvError::WriteError {
        message: format!("Failed to create temp file: {}", e),
    })?;

    file.write_all(content.as_bytes()).await.map_err(|e| CsvError::WriteError {
...
async fn scan_directory(
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| CsvError::ReadError {
...
mod tests {
    ...
    fn test_schema_path_with_spaces() {
        ...
        let schema = schema_path(&csv);
        assert_eq!(schema, PathBuf::from("/path/to/my data file.csv.vault.json"));
    }
    ...
    use crate::csv::types::{
        ColumnMetadata, ColumnSchema, DataType, DatasetMetadata, SemanticRole,
        ...
        CsvSchema {
            ...
            columns: vec![
                ...
                ColumnSchema {
                    ...
                    data_type: DataType::Currency { code: "USD".to_string() },
    ...
    async fn test_schema_save_and_load_roundtrip() {
        ...
        assert_eq!(loaded_schema.columns[0].display_name, Some("ID".to_string()));
        assert!(matches!(loaded_schema.columns[0].semantic_role, SemanticRole::Identifier));
        assert!(matches!(&loaded_schema.columns[1].data_type, DataType::Currency { code } if code == "USD"));
    ...
    async fn test_schema_roundtrip_with_relationships() {
        ...
        assert!(matches!(loaded.relationships[0].cardinality, Cardinality::ManyToOne));
    }
    ...
    async fn test_schema_roundtrip_all_data_types() {
        ...
        let schema = CsvSchema {
            ...
            columns: vec![
                ...
                ColumnSchema {
                    ...
                    data_type: DataType::Date { format: "YYYY-MM-DD".to_string() },
                ...
                ColumnSchema {
                    ...
                    data_type: DataType::DateTime { format: "YYYY-MM-DD HH:mm:ss".to_string() },
                    semantic_role: SemanticRole::Temporal,
                    format: None,
                    metadata: ColumnMetadata::default(),
                },
                ...
                ColumnSchema {
                    ...
                    data_type: DataType::Enum { values: vec!["A".to_string(), "B".to_string(), "C".to_string()] },
                    semantic_role: SemanticRole::Dimension,
                    format: None,
                    metadata: ColumnMetadata::default(),
                },
        ...
        assert!(matches!(loaded.columns[2].data_type, DataType::Decimal { precision: Some(2) }));
        assert!(matches!(loaded.columns[3].data_type, DataType::Boolean));
        assert!(matches!(loaded.columns[4].data_type, DataType::Date { .. }));
        assert!(matches!(loaded.columns[5].data_type, DataType::DateTime { .. }));
    ...
    async fn test_schema_roundtrip_all_semantic_roles() {
        ...
        let schema = CsvSchema {
            ...
            columns: vec![
                ...
                ColumnSchema {
                    ...
                    display_name: None,
                    description: "".to_string(),
                    data_type: DataType::DateTime { format: "YYYY-MM-DD".to_string() },
                    semantic_role: SemanticRole::Temporal,
                    format: None,
                    metadata: ColumnMetadata::default(),
                },
        ...
        assert!(matches!(loaded.columns[0].semantic_role, SemanticRole::Identifier));
        assert!(matches!(loaded.columns[1].semantic_role, SemanticRole::Dimension));
        assert!(matches!(loaded.columns[2].semantic_role, SemanticRole::Measure));
        assert!(matches!(loaded.columns[3].semantic_role, SemanticRole::Temporal));
        assert!(matches!(loaded.columns[4].semantic_role, SemanticRole::Descriptive));
        assert!(matches!(loaded.columns[5].semantic_role, SemanticRole::Reference { .. }));
        assert!(matches!(loaded.columns[6].semantic_role, SemanticRole::Unknown));
    }
