...
use crate::license::{get_machine_fingerprint, load_license, FEATURE_CSV_UNLIMITED_ROWS, FEATURE_CSV_PRO, FEATURE_CSV_SCHEMA, FEATURE_CSV_AI_CONTEXT};
use super::schema_store;
use super::processor;
use super::types::{CsvAiContext, CsvData, CsvError, CsvFileInfo, CsvSchema, CsvStatistics, FREE_ROW_LIMIT};
...
fn has_premium_csv_features() -> bool {
    ...
    match load_license(&machine_id) {
        Ok(Some(license_info)) => {
            ...
            license_info.features.iter().any(|f| {
                f == FEATURE_CSV_UNLIMITED_ROWS || f == FEATURE_CSV_PRO
            })
        }
        _ => false,
    }
...
fn has_csv_premium(_feature: &str) -> bool {
    ...
    match load_license(&machine_id) {
        Ok(Some(license_info)) => {
            ...
            let has_valid_license = valid_license_types.contains(&license_info.license_type.as_str());

            if !has_valid_license {
                eprintln!("[CSV Premium] Unknown license type: {}", license_info.license_type);
            }
...
fn validate_path_within_vault(path: &str, vault_path: &std::path::Path) -> Result<PathBuf, CsvError> {
    // Construct the full path
    let full_path = vault_path.join(path);

    // Canonicalize both paths to resolve symlinks and .. components
    let canonical_vault = vault_path.canonicalize().map_err(|e| {
        CsvError::ReadError {
...
/// * `Err(CsvError::PathViolation)` - If path attempts to escape vault boundary
fn validate_save_path_within_vault(path: &str, vault_path: &std::path::Path) -> Result<PathBuf, CsvError> {
    // Construct the full path
    let full_path = vault_path.join(path);
...
pub async fn save_csv_data(
    ...
    // Write to temp file
    tokio::fs::write(&temp_path, &csv_content).await.map_err(|e| {
        CsvError::WriteError {
    ...
    tokio::fs::rename(&temp_path, &full_path).await.map_err(|e| {
...
pub async fn export_to_file(
    ...
    // Write the file
    tokio::fs::write(file_path, content.as_bytes()).await.map_err(|e| {
        CsvError::WriteError {
