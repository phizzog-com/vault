...
use super::types::{Plugin, PluginManifest, PluginStatus, PluginError, PluginMetadata};
...
impl PluginScanner {
    ...
    pub async fn scan_plugins(&self) -> Result<Vec<Plugin>, PluginError> {
        ...
        println!("PluginScanner: Scanning plugins directory: {:?}", self.plugins_dir);
        
        // Ensure plugins directory exists
        if !self.plugins_dir.exists() {
            println!("PluginScanner: Plugins directory does not exist, creating: {:?}", self.plugins_dir);
        ...
        let enabled_plugins = match self.load_enabled_plugins().await {
            Ok(enabled) => {
                println!("PluginScanner: Loaded enabled plugins: {:?}", enabled.keys().collect::<Vec<_>>());
                enabled
            },
            Err(e) => {
                println!("PluginScanner: Failed to load enabled plugins: {}, using empty map", e);
                HashMap::new()
            }
        };
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                match self.load_plugin_from_dir(&path, &enabled_plugins).await {
                    Ok(plugin) => {
                        println!("PluginScanner: Successfully loaded plugin: {} v{}", plugin.name, plugin.version);
                        plugins.push(plugin);
                    },
                    Err(e) => {
    ...
    pub async fn load_plugin_from_dir(&self, dir: &Path, enabled_plugins: &HashMap<String, bool>) -> Result<Plugin, PluginError> {
        ...
        let metadata = self.load_plugin_metadata(&manifest.id).await.unwrap_or_else(|_| {
            PluginMetadata {
        ...
        let settings = self.load_plugin_settings(&manifest.id).await.unwrap_or_default();
    ...
    pub fn validate_manifest(&self, manifest: &PluginManifest) -> Result<(), PluginError> {
        ...
        if !self.is_valid_semver(&manifest.version) {
            return Err(PluginError::InvalidManifest("Invalid version format".to_string()));
        }
        ...
        for permission in &manifest.permissions {
            if permission == "system:*" || permission == "fs:write:*" {
                ...
                eprintln!("Warning: Plugin {} requests dangerous permission: {}", manifest.id, permission);
            }
        }
    ...
    pub async fn save_enabled_plugins(&self, enabled: &HashMap<String, bool>) -> Result<(), PluginError> {
        let enabled_file = self.app_data_dir.join("enabled_plugins.json");
    ...
    pub async fn save_plugin_metadata(&self, plugin_id: &str, metadata: &PluginMetadata) -> Result<(), PluginError> {
    ...
    async fn load_plugin_settings(&self, plugin_id: &str) -> Result<HashMap<String, serde_json::Value>, PluginError> {
    ...
    pub async fn save_plugin_settings(&self, plugin_id: &str, settings: &HashMap<String, serde_json::Value>) -> Result<(), PluginError> {
