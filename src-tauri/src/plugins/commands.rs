...
use super::types::{Plugin, PluginError, InstallOptions, PluginSettings};
...
impl PluginManagerState {
    ...
    pub async fn get_or_init(&self) -> Result<Arc<PluginManager>, PluginError> {
        ...
        let app_data_dir = self.app_handle.path()
            .app_data_dir()
            .map_err(|e| PluginError::IoError(std::io::Error::new(std::io::ErrorKind::Other, e.to_string())))?;
        ...
        if !plugins_dir.exists() {
            ...
            if !plugins_dir.exists() {
                ...
                if let Ok(current_dir) = std::env::current_dir() {
                    let parent_plugins = current_dir.parent()
...
pub async fn plugin_install(
    ...
    manager.install_plugin(options).await
...
pub async fn plugin_get_settings(
    ...
    manager.get_plugin_settings(&plugin_id).await
...
pub async fn plugin_update_settings(
    ...
    manager.update_plugin_settings(&settings.plugin_id, settings.settings).await
        .map_err(|e| format!("Failed to update plugin settings: {}", e))
}

// Refresh plugins list from filesystem
#[tauri::command]
pub async fn plugin_refresh(
    state: State<'_, PluginManagerState>
) -> Result<Vec<Plugin>, String> {
...
pub async fn plugin_clear_data(
    ...
    // Clear settings
    manager.update_plugin_settings(&plugin_id, HashMap::new()).await
        .map_err(|e| format!("Failed to clear plugin data: {}", e))
}

// Get all plugin resources (for compatibility)
#[tauri::command]
pub async fn plugin_get_all_resources() -> Result<HashMap<String, super::super::plugin_runtime::commands::ResourceUsage>, String> {
