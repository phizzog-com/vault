    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PathValidationError::AbsolutePath => {
                write!(f, "Absolute paths not allowed - use relative paths from vault root")
            }
        ...
        Some(window_state) => {
            ...
            match &*vault_lock {
                Some(vault) => {
                    ...
                    let tag_regex = Regex::new(r"(?:^|\s)#([a-zA-Z][a-zA-Z0-9_-]*)(?:\s|$|[.,!?;:])")
        ...
        Some(window_state) => {
            ...
            match &*vault_lock {
                Some(vault) => {
                    ...
                    let pattern = format!(r"(?:^|\s)#{}(?:\s|$|[.,!?;:])", regex::escape(tag_normalized));
