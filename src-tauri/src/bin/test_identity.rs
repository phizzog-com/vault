use vault::identity::{IdentityManager, uuid::UuidGenerator};
use std::env;
...
async fn main() {
    ...
    println!("  Before rename - ID for file1: {:?}", manager.get_note_id(&file1).unwrap());
    ...
    assert_eq!(new_id, Some(id1.clone()), "New path should have original ID");
    ...
    assert!(sidecar_content.contains(&sidecar_id), "Sidecar should contain UUID");
