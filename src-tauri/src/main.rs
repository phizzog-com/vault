...
use commands::mcp_config::{generate_mcp_config, write_mcp_config, validate_mcp_server};
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    if let Ok(saved_state) = crate::window_lifecycle::AppPersistenceState::load() {
                        if let Some(window_state) = saved_state.last_active_window {
                            ...
                            let (screen_width, screen_height) = window_clone.primary_monitor()
                                .ok()
                                .flatten()
                                .map(|m| {
                                    let size = m.size();
                            ...
                            let height = window_state.bounds.height.clamp(min_height, screen_height);
                            ...
                            println!("🪟 Restoring window: {}x{} at ({}, {}) [screen: {}x{}]",
