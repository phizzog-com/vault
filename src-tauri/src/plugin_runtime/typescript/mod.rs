...
impl TypeScriptGenerator {
    ...
    pub async fn generate_module_types(&self, module: &ApiModule) -> Result<String, TypeScriptError> {
        let mut result = String::new();
        
        // Module interface
        result.push_str(&format!("/**\n * {}\n */\n", module.description));
        result.push_str(&format!("export interface {}API {{\n", 
            module.name.chars().next().unwrap().to_uppercase().to_string() + &module.name[1..]
        ));
        ...
        for method in &module.methods {
            ...
            let jsdoc = JsDocComment {
                ...
                params: method.params.iter().map(|p| JsDocParam {
                    name: p.name.clone(),
...
"#.to_string())
    ...
    pub async fn validate_typescript(&self, content: &str) -> Result<(), TypeScriptError> {
        ...
        if brace_count != 0 {
            return Err(TypeScriptError::InvalidSyntax("Unmatched braces".to_string()));
        }
        
        if in_string {
            return Err(TypeScriptError::InvalidSyntax("Unclosed string".to_string()));
        }
        
        // Check for missing types in interface
        if content.contains("interface") && content.contains("value  //") {
            return Err(TypeScriptError::InvalidSyntax("Missing type definition".to_string()));
        }
    ...
    pub async fn export_definitions(&self, api: &ApiStructure, output_path: &Path) -> Result<(), TypeScriptError> {
    ...
    pub async fn export_ambient_declarations(&self, output_path: &Path) -> Result<(), TypeScriptError> {
        let content = r#"/**
...
"#;
    ...
    pub async fn generate_migration_guide(&self, from_platform: &str) -> Result<String, TypeScriptError> {
        match from_platform {
            "obsidian" => Ok(self.generate_obsidian_migration_guide()),
            _ => Err(TypeScriptError::GenerationFailed(format!("Unknown platform: {}", from_platform))),
        }
    }
...
"#.to_string()
    ...
    pub async fn generate_type_package(&self, api: &ApiStructure, output_dir: &Path) -> Result<(), TypeScriptError> {
        // Create directory
        fs::create_dir_all(output_dir)?;
        
        // Generate type definitions
        self.export_definitions(api, &output_dir.join("index.d.ts")).await?;
        
        // Generate ambient declarations
        self.export_ambient_declarations(&output_dir.join("global.d.ts")).await?;
        
        // Generate tsconfig.json
        self.generate_tsconfig(&output_dir.join("tsconfig.json")).await?;
        
        // Generate package.json
        self.generate_package_json(&output_dir.join("package.json")).await?;
    ...
    pub async fn generate_vscode_settings(&self, vscode_dir: &Path) -> Result<(), TypeScriptError> {
        ...
        fs::write(vscode_dir.join("settings.json"), serde_json::to_string_pretty(&settings)?)?;
