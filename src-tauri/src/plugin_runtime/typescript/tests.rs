...
async fn test_generate_tsconfig() {
    ...
    assert!(json["compilerOptions"]["lib"].as_array().unwrap().contains(&serde_json::json!("ES2020")));
}
...
async fn test_vault_api_contains_required_methods() {
    ...
    let method_names: Vec<&str> = vault_module.methods.iter().map(|m| m.name.as_str()).collect();
