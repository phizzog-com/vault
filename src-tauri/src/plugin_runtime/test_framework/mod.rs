...
impl PluginTestHarness {
    ...
    pub fn get_test_count(&self) -> usize {
        futures::executor::block_on(async {
            *self.test_count.lock().await
        })
    }
    
    /// Load plugin
    pub async fn load_plugin(&mut self, id: &str, code: String, manifest: Value) -> Result<String, TestError> {
    ...
    pub async fn execute_plugin(&mut self, id: &str) -> Result<ExecutionResult, TestError> {
        ...
        let (plugin_code, should_error, should_timeout) = {
            ...
            let has_timeout = code.contains("setTimeout(resolve, 1000)") && self.config.timeout_ms < 1000;
        ...
        self.mock_settings.set("test-key", serde_json::json!("test-value")).await.ok();
    ...
    /// Call plugin method
    pub async fn call_plugin_method(&mut self, id: &str, method: &str, args: Vec<Value>) -> Result<Value, TestError> {
        // Mock implementation
    ...
    /// Run test suite
    pub async fn run_test_suite(&mut self, _suite: TestSuite) -> Result<TestSuiteReport, TestError> {
        // Mock implementation
    ...
    pub async fn run_test_suites_parallel(&mut self, suites: Vec<TestSuite>) -> Result<Vec<TestSuiteReport>, TestError> {
...
impl MockMcpApi {
    ...
    {
        self.tools.write().await.insert(name.to_string(), Box::new(handler));
    }
    ...
    pub async fn call_tool(&self, name: &str, params: Value) -> Result<Value, TestError> {
        ...
        if let Some(handler) = tools.get(name) {
            ...
            Err(TestError::ExecutionFailed(format!("Tool not found: {}", name)))
        }
    }
}
...
impl MockNetworkApi {
    ...
    pub async fn fetch(&self, url: &str, _options: Value) -> Result<MockResponse, TestError> {
        self.responses.read().await.get(url)
            .cloned()
    ...
    pub async fn advance(&mut self, ms: u64) {
        if matches!(self.mode, TimeControl::Manual | TimeControl::Accelerated(_)) {
            ...
            *self.current_time.write().await = new_time;
            ...
            let mut to_execute: Vec<(u64, Box<dyn FnOnce() + Send>)> = Vec::new();
            
            tasks.retain(|(time, _task)| {
                if *time <= new_time {
                    false
                } else {
                    true
...
impl Assertions {
    ...
    pub fn assert_equals<T: PartialEq + std::fmt::Debug>(&self, actual: T, expected: T, message: &str) {
        if actual != expected {
            panic!("Assertion failed: {} - expected {:?}, got {:?}", message, expected, actual);
        }
    }
    
    pub fn assert_not_equals<T: PartialEq + std::fmt::Debug>(&self, actual: T, expected: T, message: &str) {
        if actual == expected {
            panic!("Assertion failed: {} - values should not be equal: {:?}", message, actual);
        }
    }
    
    pub fn assert_greater_than<T: PartialOrd + std::fmt::Debug>(&self, actual: T, expected: T, message: &str) {
        if actual <= expected {
            panic!("Assertion failed: {} - {:?} should be greater than {:?}", message, actual, expected);
        }
    }
    
    pub fn assert_contains(&self, haystack: &str, needle: &str, message: &str) {
        if !haystack.contains(needle) {
            panic!("Assertion failed: {} - '{}' should contain '{}'", message, haystack, needle);
        }
    }
