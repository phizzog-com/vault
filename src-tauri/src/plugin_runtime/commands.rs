...
static ENABLED_PLUGINS: Lazy<Mutex<HashSet<String>>> = Lazy::new(|| {
                    ...
                    if manifest_path.exists() {
                        ...
                        if let Ok(manifest_content) = std::fs::read_to_string(&manifest_path) {
                            if let Ok(manifest) = serde_json::from_str::<serde_json::Value>(&manifest_content) {
                                ...
                                let is_enabled = ENABLED_PLUGINS.lock().unwrap().contains(&plugin_id.to_string());
                                ...
                                let plugin_info = PluginInfo {
                                    ...
                                    status: if is_enabled { PluginStatus::Active } else { PluginStatus::Inactive },
                                    icon: None,
                                    homepage: manifest["repository"].as_str().map(String::from),
                                };
                                
                                plugins.push(plugin_info);
                                println!("Found plugin: {}", manifest["name"].as_str().unwrap_or("Unknown"));
                            }
                        }
                    }
                }
...
pub async fn _old_plugin_install(
...
// Enable a plugin
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_enable(
    plugin_id: String
) -> Result<(), String> {
...
// Disable a plugin
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_disable(
    plugin_id: String
) -> Result<(), String> {
...
// Uninstall a plugin
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_uninstall(
    plugin_id: String
) -> Result<(), String> {
    // let mut runtime = runtime.lock().await;
...
pub async fn _old_plugin_get_settings(
    ...
    if plugin_id == "readwise" {
        ...
        settings.insert("apiToken".to_string(), serde_json::Value::String("".to_string()));
        settings.insert("syncFrequency".to_string(), serde_json::Value::Number(60.into()));
        settings.insert("autoSync".to_string(), serde_json::Value::Bool(false));
        settings.insert("highlightsFolder".to_string(), serde_json::Value::String("Readwise".to_string()));
        settings.insert("groupBy".to_string(), serde_json::Value::String("book".to_string()));
        return Ok(settings);
    }
...
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_update_settings(
...
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_get_resources(
    plugin_id: String
...
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_request_permission(
    ...
    println!("Permission request for {}: {}", request.plugin_id, request.permission);
...
// Clear plugin data
// #[tauri::command] // DEPRECATED
pub async fn _old_plugin_clear_data(
    plugin_id: String
) -> Result<(), String> {
    // let mut runtime = runtime.lock().await;
