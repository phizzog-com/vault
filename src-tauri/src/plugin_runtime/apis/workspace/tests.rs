...
mod workspace_api_tests {
    ...
    async fn grant_workspace_permission(api: &WorkspaceApi, plugin_id: &str, permission: WorkspacePermission) {
    ...
    mod active_file_management {
        ...
        async fn test_set_active_file_without_permission() {
            ...
            assert!(matches!(result.unwrap_err(), WorkspaceError::PermissionDenied(_)));
        }
        ...
        async fn test_open_file() {
            ...
            // Open a file
            let result = workspace.open_file("test-plugin", "document.md", false).await;
            assert!(result.is_ok());
        ...
        async fn test_close_file() {
            ...
            workspace.open_file("test-plugin", "temp.md", false).await.unwrap();
    ...
    mod view_management {
        ...
        async fn test_create_view() {
            let workspace = create_test_workspace().await;
            grant_workspace_permission(&workspace, "test-plugin", WorkspacePermission::Create).await;
        ...
        async fn test_destroy_view() {
            ...
            let view_id = workspace.create_view("test-plugin", view_config).await.unwrap();
        ...
        async fn test_get_active_view() {
            ...
            let view_id = workspace.create_view("test-plugin", view_config.clone()).await.unwrap();
        ...
        async fn test_list_views() {
            ...
            for i in 0..3 {
                ...
                workspace.create_view("test-plugin", view_config).await.unwrap();
            }
    ...
    mod layout_management {
        ...
        async fn test_split_editor() {
            ...
            let result = workspace.split_editor("test-plugin", SplitDirection::Horizontal).await;
            assert!(result.is_ok());
        }
        ...
        async fn test_toggle_sidebar() {
            ...
            // Toggle left sidebar
            let result = workspace.toggle_sidebar("test-plugin", SidebarPosition::Left).await;
            assert!(result.is_ok());
            
            // Toggle right sidebar
            let result = workspace.toggle_sidebar("test-plugin", SidebarPosition::Right).await;
            assert!(result.is_ok());
        }
    }
    ...
    mod modal_and_notices {
        ...
        async fn test_close_modal() {
            ...
            let modal_id = workspace.show_modal("test-plugin", modal_config).await.unwrap();
        ...
        async fn test_show_notice() {
            ...
            // Show info notice
            let result = workspace.show_notice(
                "test-plugin",
                NoticeType::Info,
                "Test notice",
                Some(5000),
    ...
    mod status_bar_and_ribbon {
        ...
        async fn test_add_status_bar_item() {
            ...
            let result = workspace.add_status_bar_item("test-plugin", status_item).await;
            assert!(result.is_ok());
        }
        ...
        async fn test_update_status_bar_item() {
            ...
            workspace.add_status_bar_item("test-plugin", status_item).await.unwrap();
            
            // Update it
            let result = workspace.update_status_bar_item(
                "test-plugin",
                "update-status",
                Some("Updated Text"),
                None,
        ...
        async fn test_remove_status_bar_item() {
            ...
            // Remove it
            let result = workspace.remove_status_bar_item("test-plugin", "remove-status").await;
            assert!(result.is_ok());
        }
    ...
    mod workspace_events {
        ...
        async fn test_subscribe_to_events() {
            ...
            let result = workspace.subscribe_to_event(
                "test-plugin",
                WorkspaceEventType::FileOpened,
                tx.clone(),
            ).await;
            assert!(result.is_ok());
            
            // Trigger an event
            workspace.emit_event_internal(WorkspaceEvent {
        ...
        async fn test_unsubscribe_from_events() {
            ...
            // Subscribe
            workspace.subscribe_to_event(
                "test-plugin",
                WorkspaceEventType::FileClosed,
                tx,
            ).await.unwrap();
            
            // Unsubscribe
            let result = workspace.unsubscribe_from_event(
                "test-plugin",
                WorkspaceEventType::FileClosed,
            ).await;
            assert!(result.is_ok());
        ...
        async fn test_command_unregistration() {
            ...
            workspace.register_command("test-plugin", command).await.unwrap();
            
            // Unregister it
            let result = workspace.unregister_command("test-plugin", "temp-command").await;
            assert!(result.is_ok());
        }
    }
