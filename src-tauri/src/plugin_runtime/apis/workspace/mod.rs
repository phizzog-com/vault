...
struct WorkspaceState {
    ...
    event_subscribers: HashMap<WorkspaceEventType, Vec<(String, tokio::sync::mpsc::Sender<WorkspaceEvent>)>>,
}
...
impl WorkspaceApi {
    ...
    async fn check_permission(&self, plugin_id: &str, permission: WorkspacePermission) -> Result<(), WorkspaceError> {
        let capability = match permission {
            WorkspacePermission::Read => Capability::WorkspaceRead,
            WorkspacePermission::Write => Capability::WorkspaceWrite,
            WorkspacePermission::Create => Capability::WorkspaceCreate,
    ...
    pub async fn get_active_file(&self, plugin_id: &str) -> Result<Option<String>, WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Read).await?;
    ...
    pub async fn set_active_file(&self, plugin_id: &str, path: &str) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Write).await?;
        ...
        self.emit_event_internal(WorkspaceEvent {
            ...
            data: WorkspaceEventData::File { path: path.to_string() },
    ...
    pub async fn open_file(&self, plugin_id: &str, path: &str, _new_pane: bool) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Write).await?;
    ...
    /// Create a new view
    pub async fn create_view(&self, plugin_id: &str, config: ViewConfig) -> Result<String, WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
        ...
        self.emit_event_internal(WorkspaceEvent {
            ...
            data: WorkspaceEventData::View { id: view_id.clone() },
            timestamp: chrono::Utc::now().timestamp() as u64,
        }).await;
    ...
    pub async fn destroy_view(&self, plugin_id: &str, view_id: &str) -> Result<(), WorkspaceError> {
        ...
        self.emit_event_internal(WorkspaceEvent {
            ...
            data: WorkspaceEventData::View { id: view_id.to_string() },
            timestamp: chrono::Utc::now().timestamp() as u64,
        }).await;
    ...
    /// Get the active view
    pub async fn get_active_view(&self, plugin_id: &str) -> Result<Option<ViewInfo>, WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Read).await?;
    ...
    /// Split the editor
    pub async fn split_editor(&self, plugin_id: &str, _direction: SplitDirection) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Write).await?;
    ...
    pub async fn focus_pane(&self, plugin_id: &str, _pane_index: usize) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Write).await?;
    ...
    pub async fn toggle_sidebar(&self, plugin_id: &str, position: SidebarPosition) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Write).await?;
    ...
    /// Show a modal dialog
    pub async fn show_modal(&self, plugin_id: &str, config: ModalConfig) -> Result<String, WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
    ...
    pub async fn add_status_bar_item(&self, plugin_id: &str, item: StatusBarItem) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
    ...
    pub async fn remove_status_bar_item(&self, plugin_id: &str, item_id: &str) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
    ...
    /// Add a ribbon item
    pub async fn add_ribbon_item(&self, plugin_id: &str, item: RibbonItem) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
    ...
    pub async fn subscribe_to_event(
        ...
        let mut state = self.state.write().await;
        let subscribers = state.event_subscribers.entry(event_type).or_insert_with(Vec::new);
    ...
    /// Register a command
    pub async fn register_command(&self, plugin_id: &str, command: Command) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
    ...
    pub async fn unregister_command(&self, plugin_id: &str, command_id: &str) -> Result<(), WorkspaceError> {
        self.check_permission(plugin_id, WorkspacePermission::Create).await?;
