...
mod vault_api_tests {
    ...
    mod file_operations {
        ...
        async fn test_read_file_with_permission() {
            ...
            vault_api.grant_permission("test-plugin", VaultPermission::Read).await;
        ...
        async fn test_read_file_without_permission() {
            ...
            assert!(matches!(result.unwrap_err(), VaultError::PermissionDenied(_)));
        }
        ...
        async fn test_write_file_with_permission() {
            ...
            vault_api.grant_permission("test-plugin", VaultPermission::Write).await;
        ...
        async fn test_append_to_file() {
            ...
            // Append to file
            let result = vault_api.append("test-plugin", "append.md", " content").await;
            assert!(result.is_ok());
        ...
        async fn test_delete_file() {
            ...
            vault_api.grant_permission("test-plugin", VaultPermission::Delete).await;
    ...
    mod directory_operations {
        ...
        async fn test_delete_directory() {
            ...
            // Delete directory
            let result = vault_api.delete_folder("test-plugin", "to_delete", false).await;
            assert!(result.is_ok());
        ...
        async fn test_recursive_delete() {
            ...
            let result = vault_api.delete_folder("test-plugin", "nested", false).await;
            assert!(result.is_err());
    ...
    mod path_validation {
        ...
        async fn test_symlink_resolution() {
            let (vault_api, temp_dir) = create_test_vault().await;
            create_test_file(&temp_dir.path().to_path_buf(), "target.md", "target content");
    ...
    mod binary_operations {
        ...
        async fn test_write_binary_file() {
            ...
            let result = vault_api.write_binary("test-plugin", "image.png", binary_content.clone()).await;
            assert!(result.is_ok());
        ...
        async fn test_large_file_handling() {
            ...
            // Write large file
            let write_result = vault_api.write_binary("test-plugin", "large.bin", large_content.clone()).await;
    ...
    mod file_watching {
        ...
        async fn test_watch_event_coalescing() {
            ...
            let (tx, mut rx) = tokio::sync::mpsc::channel(10);
            vault_api.watch("test-plugin", "rapid.md", tx).await.unwrap();
    ...
    mod mcp_integration {
        ...
        async fn test_mcp_permission_consistency() {
            ...
            vault_api.set_mcp_mode(false).await;
            let direct_result = vault_api.write("test-plugin", "direct2.md", "content").await;
    ...
    mod performance {
        ...
        async fn test_list_performance() {
            ...
            // Create 1000 files
            for i in 0..1000 {
                create_test_file(&temp_dir.path().to_path_buf(), &format!("file{}.md", i), "content");
            }
