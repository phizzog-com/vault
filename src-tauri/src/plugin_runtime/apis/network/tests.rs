...
mod network_api_tests {
    ...
    async fn grant_network_permission(api: &NetworkApi, plugin_id: &str, permission: NetworkPermission) {
    ...
    mod fetch_operations {
        ...
        async fn test_fetch_with_allowed_domain() {
            ...
            network.grant_domain_permission("test-plugin", "api.example.com").await;
        ...
        async fn test_fetch_with_disallowed_domain() {
            ...
            assert!(matches!(result.unwrap_err(), NetworkError::DomainNotAllowed(_)));
        }
        ...
        async fn test_fetch_without_permission() {
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::PermissionDenied(_)));
        }
        ...
        async fn test_all_http_methods() {
            ...
            for method in methods {
                let request = FetchRequest {
                    ...
                    method: method.clone(),
                    headers: HashMap::new(),
                    body: if matches!(method, HttpMethod::Post | HttpMethod::Put | HttpMethod::Patch) {
                        Some("{}".to_string())
                    } else {
                        None
                    },
    ...
    mod https_enforcement {
        ...
        async fn test_https_required() {
            let network = create_test_network().await;
            grant_network_permission(&network, "test-plugin", NetworkPermission::Fetch).await;
            network.grant_domain_permission("test-plugin", "example.com").await;
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::InsecureProtocol(_)));
        }
        ...
        async fn test_localhost_exception() {
            let network = create_test_network().await;
            grant_network_permission(&network, "test-plugin", NetworkPermission::Fetch).await;
            network.grant_domain_permission("test-plugin", "localhost").await;
    ...
    mod size_limits {
        ...
        async fn test_request_size_limit() {
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::RequestTooLarge(_)));
        }
        ...
        async fn test_response_size_limit() {
            ...
            network.set_mock_response("https://api.example.com/large", MockResponse {
                status: 200,
                headers: HashMap::new(),
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::ResponseTooLarge(_)));
        }
    ...
    mod websocket_support {
        ...
        async fn test_websocket_connection_with_permission() {
            ...
            network.grant_domain_permission("test-plugin", "ws.example.com").await;
            
            let result = network.connect_websocket(
                "test-plugin",
        ...
        async fn test_websocket_send_receive() {
            ...
            let ws_id = network.connect_websocket(
                "test-plugin",
                "wss://ws.example.com/socket",
            ).await.unwrap();
            
            // Send message
            let send_result = network.websocket_send(
                "test-plugin",
                &ws_id,
            ...
            network.websocket_subscribe("test-plugin", &ws_id, tx).await.unwrap();
            
            // Simulate incoming message
            network.emit_websocket_message_internal(
                &ws_id,
    ...
    mod domain_allowlisting {
        ...
        async fn test_ip_literal_blocking() {
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::IpLiteralNotAllowed(_)));
        }
        ...
        async fn test_redirect_handling() {
            ...
            network.set_mock_redirect("https://example.com/start", "https://example.com/redirect1").await;
            network.set_mock_redirect("https://example.com/redirect1", "https://evil.com/final").await;
            ...
            assert!(result.is_ok() || matches!(result.err(), Some(NetworkError::RedirectToDisallowedDomain(_))));
        }
    }
    ...
    mod rate_limiting {
        ...
        async fn test_request_rate_limiting() {
            ...
            network.set_rate_limit("test-plugin", 5, Duration::from_secs(1)).await;
        ...
        async fn test_per_domain_rate_limits() {
            let network = create_test_network().await;
            grant_network_permission(&network, "test-plugin", NetworkPermission::Fetch).await;
            network.grant_domain_permission("test-plugin", "api1.example.com").await;
            network.grant_domain_permission("test-plugin", "api2.example.com").await;
            
            // Set different rate limits per domain
            network.set_domain_rate_limit("test-plugin", "api1.example.com", 2, Duration::from_secs(1)).await;
            network.set_domain_rate_limit("test-plugin", "api2.example.com", 5, Duration::from_secs(1)).await;
    ...
    mod telemetry_and_privacy {
        ...
        async fn test_sensitive_header_redaction() {
            ...
            assert_eq!(log[0].headers.get("Authorization"), Some(&"[REDACTED]".to_string()));
            assert_eq!(log[0].headers.get("X-API-Key"), Some(&"[REDACTED]".to_string()));
            assert_eq!(log[0].headers.get("Content-Type"), Some(&"application/json".to_string()));
        }
    }
    ...
    mod error_handling {
        ...
        async fn test_timeout_handling() {
            let network = create_test_network().await;
            grant_network_permission(&network, "test-plugin", NetworkPermission::Fetch).await;
            network.grant_domain_permission("test-plugin", "slow.example.com").await;
            
            // Set mock to timeout
            network.set_mock_timeout("https://slow.example.com/api", true).await;
        ...
        async fn test_dns_failure() {
            let network = create_test_network().await;
            grant_network_permission(&network, "test-plugin", NetworkPermission::Fetch).await;
            network.grant_domain_permission("test-plugin", "nonexistent.example.com").await;
            
            // Set mock DNS failure
            network.set_mock_dns_failure("nonexistent.example.com", true).await;
            ...
            let result = network.fetch("test-plugin", request).await;
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), NetworkError::DnsResolutionFailed(_)));
        }
