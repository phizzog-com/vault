...
mod settings_api_tests {
    ...
    async fn grant_settings_permission(api: &SettingsApi, plugin_id: &str, permission: SettingsPermission) {
    ...
    mod basic_storage {
        ...
        async fn test_delete_value() {
            ...
            // Set and then delete
            settings.set("test-plugin", "temp", "temporary").await.unwrap();
        ...
        async fn test_permission_denial() {
            ...
            assert!(matches!(result.unwrap_err(), SettingsError::PermissionDenied(_)));
        }
    }
    ...
    mod namespaced_storage {
        ...
        async fn test_plugin_isolation() {
            ...
            settings.set("plugin-a", "shared-key", "value-a").await.unwrap();
            settings.set("plugin-b", "shared-key", "value-b").await.unwrap();
        ...
        async fn test_cross_plugin_access_denied() {
            ...
            grant_settings_permission(&settings, "plugin-a", SettingsPermission::Write).await;
            settings.set("plugin-a", "secret", "plugin-a-secret").await.unwrap();
    ...
    mod complex_data_types {
        ...
        async fn test_json_storage() {
            ...
            let result = settings.set_json("test-plugin", "config", json_data.clone()).await;
            assert!(result.is_ok());
    ...
    mod storage_quota {
        ...
        async fn test_quota_enforcement() {
            ...
            assert!(result.is_err());
            assert!(matches!(result.unwrap_err(), SettingsError::QuotaExceeded(_)));
        }
        ...
        async fn test_quota_cleanup() {
            ...
            settings.set("test-plugin", "key1", "x".repeat(30).as_str()).await.unwrap();
            settings.set("test-plugin", "key2", "y".repeat(30).as_str()).await.unwrap();
            
            // Delete one key to free space
            settings.delete("test-plugin", "key1").await.unwrap();
            
            // Should now have space for new data
            let result = settings.set("test-plugin", "key3", "z".repeat(20).as_str()).await;
            assert!(result.is_ok());
        }
    }
    ...
    mod migration_system {
        ...
        async fn test_version_migration() {
            ...
            settings.set("test-plugin", "old_key", "old_value").await.unwrap();
            ...
            // Apply migration
            settings.apply_migration("test-plugin", migration).await.unwrap();
    ...
    mod encryption {
        ...
        async fn test_encrypted_storage() {
            ...
            let result = settings.set_encrypted("test-plugin", "api_key", secret).await;
            assert!(result.is_ok());
            ...
            let raw = settings.get_raw_internal("test-plugin", "api_key").await.unwrap();
        ...
        async fn test_encryption_key_rotation() {
            ...
            settings.set_encrypted("test-plugin", "secret", "my-secret").await.unwrap();
            
            // Rotate encryption key
            settings.rotate_encryption_key("test-plugin").await.unwrap();
            
            // Should still be able to decrypt
            let decrypted = settings.get_encrypted("test-plugin", "secret").await.unwrap();
    ...
    mod cleanup {
        ...
        async fn test_plugin_cleanup() {
            ...
            settings.set("test-plugin", "key1", "value1").await.unwrap();
            settings.set("test-plugin", "key2", "value2").await.unwrap();
            settings.set_json("test-plugin", "config", serde_json::json!({"test": true})).await.unwrap();
    ...
    mod persistence {
        ...
        async fn test_persistence_across_restarts() {
            ...
            // First session
            {
                ...
                grant_settings_permission(&settings, "test-plugin", SettingsPermission::Write).await;
                settings.set("test-plugin", "persistent", "value").await.unwrap();
        ...
        async fn test_atomic_writes() {
            ...
            for i in 0..10 {
                ...
                let handle = tokio::spawn(async move {
                    settings_clone.set("test-plugin", &format!("key{}", i), &format!("value{}", i)).await
                });
                handles.push(handle);
            }
    ...
    mod performance {
        ...
        async fn test_write_performance() {
            ...
            for i in 0..100 {
                settings.set("test-plugin", &format!("key{}", i), &format!("value{}", i)).await.unwrap();
            }
        ...
        async fn test_read_performance() {
            ...
            // Read all values
            for i in 0..100 {
                settings.get("test-plugin", &format!("key{}", i)).await.unwrap();
            }
