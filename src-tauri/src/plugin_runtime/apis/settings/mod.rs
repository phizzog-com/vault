...
impl SettingsApi {
    ...
    pub async fn grant_permission(&self, plugin_id: &str, permission: SettingsPermission) {
        let capability = match permission {
            SettingsPermission::Read => Capability::SettingsRead { keys: vec!["*".to_string()] },
            SettingsPermission::Write => Capability::SettingsWrite { keys: vec!["*".to_string()] },
        };
    ...
    async fn check_permission(&self, plugin_id: &str, permission: SettingsPermission) -> Result<(), SettingsError> {
        let capability = match permission {
            SettingsPermission::Read => Capability::SettingsRead { keys: vec!["*".to_string()] },
            SettingsPermission::Write => Capability::SettingsWrite { keys: vec!["*".to_string()] },
        };
    ...
    async fn save_storage(&self, plugin_id: &str, storage: &PluginStorage) -> Result<(), SettingsError> {
        // Update cache
        let mut cache = self.storage_cache.write().await;
        cache.insert(plugin_id.to_string(), storage.clone());
    ...
    pub async fn set(&self, plugin_id: &str, key: &str, value: &str) -> Result<(), SettingsError> {
        self.check_permission(plugin_id, SettingsPermission::Write).await?;
        ...
        if new_size > storage.metadata.quota_bytes {
            return Err(SettingsError::QuotaExceeded(format!(
                "Storage quota exceeded: {} > {} bytes", new_size, storage.metadata.quota_bytes
            )));
        }
    ...
    pub async fn get(&self, plugin_id: &str, key: &str) -> Result<Option<String>, SettingsError> {
        self.check_permission(plugin_id, SettingsPermission::Read).await?;
    ...
    /// Set a JSON value
    pub async fn set_json(&self, plugin_id: &str, key: &str, value: JsonValue) -> Result<(), SettingsError> {
    ...
    /// Get a JSON value
    pub async fn get_json(&self, plugin_id: &str, key: &str) -> Result<Option<JsonValue>, SettingsError> {
    ...
    pub async fn set_encrypted(&self, plugin_id: &str, key: &str, value: &str) -> Result<(), SettingsError> {
        self.check_permission(plugin_id, SettingsPermission::Write).await?;
    ...
    pub async fn get_encrypted(&self, plugin_id: &str, key: &str) -> Result<Option<String>, SettingsError> {
        self.check_permission(plugin_id, SettingsPermission::Read).await?;
        ...
        if let Some(stored) = storage.encrypted_data.get(key) {
            if stored.len() < 12 {
                return Err(SettingsError::EncryptionError("Invalid encrypted data".to_string()));
            }
    ...
    /// Apply a migration
    pub async fn apply_migration(&self, plugin_id: &str, migration: Migration) -> Result<(), SettingsError> {
        self.check_permission(plugin_id, SettingsPermission::Write).await?;
    ...
    #[cfg(test)]
    pub async fn get_raw(&self, _requestor: &str, plugin_id: &str, key: &str) -> Result<Option<String>, SettingsError> {
        // Cross-plugin access is denied
        if _requestor != plugin_id {
            return Err(SettingsError::PermissionDenied("Cross-plugin access denied".to_string()));
        }
        self.get(plugin_id, key).await
    }
    
    #[cfg(test)]
    pub async fn get_raw_internal(&self, plugin_id: &str, key: &str) -> Result<Option<String>, SettingsError> {
        let storage = self.load_storage(plugin_id).await?;
