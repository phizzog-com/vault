...
mod mcp_api_tests {
    ...
    async fn setup_mock_server(api: &McpApi) {
        api.register_mock_server("test-server", MockServerConfig {
            tools: vec![
                ToolInfo {
    ...
    mod resource_operations {
        ...
        async fn test_subscribe_to_resource() {
            ...
            let result = mcp.subscribe_to_resource(
                "test-plugin",
                "test-server",
    ...
    mod prompt_operations {
        ...
        async fn test_get_prompt_with_permission() {
            ...
            let args = std::collections::HashMap::from([
                ("text".to_string(), "Long text to summarize...".to_string()),
            ]);
            
            let result = mcp.get_prompt(
                "test-plugin",
                "test-server",
                "summarize",
                args,
    ...
    mod error_handling {
        ...
        async fn test_server_timeout() {
            ...
            mcp.register_mock_server("slow-server", MockServerConfig {
                tools: vec![
                    ToolInfo {
        ...
        async fn test_invalid_server_response() {
            ...
            mcp.register_mock_server("broken-server", MockServerConfig {
                tools: vec![],
                resources: vec![
                    ResourceInfo {
    ...
    mod permission_validation {
        ...
        async fn test_tool_permission_scoping() {
            ...
            mcp.grant_tool_permission("test-plugin", "test-server", vec!["calculate"]).await;
        ...
        async fn test_server_permission_isolation() {
            ...
            mcp.grant_server_permission("test-plugin", "test-server").await;
            
            setup_mock_server(&mcp).await;
            
            // Setup another server
            mcp.register_mock_server("other-server", MockServerConfig {
                tools: vec![
                    ToolInfo {
    ...
    mod rate_limiting {
        ...
        async fn test_rate_limiting() {
            ...
            // Set rate limit
            mcp.set_rate_limit("test-plugin", 5, Duration::from_secs(1)).await;
