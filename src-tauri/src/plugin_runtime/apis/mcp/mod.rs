...
impl McpApi {
    ...
    pub async fn grant_permission(&self, plugin_id: &str, permission: McpPermission) {
        let capability = match permission {
            McpPermission::List => Capability::McpInvoke { tools: vec!["list".to_string()] },
            McpPermission::Read => Capability::McpInvoke { tools: vec!["read".to_string()] },
            McpPermission::Invoke => Capability::McpInvoke { tools: vec!["*".to_string()] },
            McpPermission::Subscribe => Capability::McpInvoke { tools: vec!["subscribe".to_string()] },
            McpPermission::Register => Capability::McpInvoke { tools: vec!["register".to_string()] },
        };
    ...
    async fn check_permission(&self, plugin_id: &str, permission: McpPermission) -> Result<(), McpError> {
        let capability = match permission {
            McpPermission::List => Capability::McpInvoke { tools: vec!["list".to_string()] },
            McpPermission::Read => Capability::McpInvoke { tools: vec!["read".to_string()] },
            McpPermission::Invoke => Capability::McpInvoke { tools: vec!["*".to_string()] },
    ...
    async fn check_rate_limit(&self, plugin_id: &str) -> Result<(), McpError> {
        ...
        if let Some(limit_info) = state.rate_limits.get_mut(plugin_id) {
            ...
            limit_info.requests.retain(|&req_time| {
                now.duration_since(req_time) < limit_info.window
            });
    ...
    pub async fn list_servers(&self, plugin_id: &str) -> Result<Vec<ServerInfo>, McpError> {
        self.check_permission(plugin_id, McpPermission::List).await?;
        
        let state = self.state.read().await;
        
        // Filter servers based on permissions
        let servers: Vec<ServerInfo> = if let Some(allowed) = state.server_permissions.get(plugin_id) {
    ...
    pub async fn get_server_info(&self, plugin_id: &str, server_name: &str) -> Result<ServerInfo, McpError> {
        self.check_permission(plugin_id, McpPermission::List).await?;
        ...
        if let Some(allowed) = state.server_permissions.get(plugin_id) {
            if !allowed.contains(&server_name.to_string()) {
                return Err(McpError::PermissionDenied(format!(
                    "Plugin {} not allowed to access server {}", plugin_id, server_name
                )));
            }
        }
    ...
    pub async fn register_server(&self, plugin_id: &str, config: ServerConfig) -> Result<(), McpError> {
        self.check_permission(plugin_id, McpPermission::Register).await?;
    ...
    pub async fn list_tools(&self, plugin_id: &str, server_name: Option<&str>) -> Result<Vec<ToolInfo>, McpError> {
        self.check_permission(plugin_id, McpPermission::List).await?;
        ...
        let tools = if let Some(server) = server_name {
            ...
            if let Some(allowed) = state.server_permissions.get(plugin_id) {
                if !allowed.contains(&server.to_string()) {
                    return Err(McpError::PermissionDenied(format!(
                        "Plugin {} not allowed to access server {}", plugin_id, server
                    )));
                }
            }
            
            state.tools.get(server)
                .cloned()
                .unwrap_or_default()
        } else {
    ...
    pub async fn invoke_tool(
        ...
        self.check_permission(plugin_id, McpPermission::Invoke).await?;
        ...
        if let Some(server_perms) = state.tool_permissions.get(plugin_id) {
            if let Some(allowed_tools) = server_perms.get(server_name) {
                if !allowed_tools.contains(&tool_name.to_string()) && !allowed_tools.contains(&"*".to_string()) {
                    return Err(McpError::PermissionDenied(format!(
        ...
        #[cfg(test)]
        {
            if let Some(&should_timeout) = state.mock_timeouts.get(server_name) {
                if should_timeout {
                    return Err(McpError::Timeout(format!("Server {} timed out", server_name)));
                }
            }
        }
    ...
    pub async fn list_resources(&self, plugin_id: &str, server_name: Option<&str>) -> Result<Vec<ResourceInfo>, McpError> {
        self.check_permission(plugin_id, McpPermission::List).await?;
        
        let state = self.state.read().await;
        
        let resources = if let Some(server) = server_name {
            state.resources.get(server)
                .cloned()
                .unwrap_or_default()
        } else {
    ...
    pub async fn read_resource(
        ...
        self.check_permission(plugin_id, McpPermission::Read).await?;
        self.check_rate_limit(plugin_id).await?;
        ...
        #[cfg(test)]
        {
            if let Some(&should_fail) = state.mock_invalid_responses.get(server_name) {
                if should_fail {
                    return Err(McpError::InvalidResponse(format!("Invalid response from server {}", server_name)));
                }
            }
        }
    ...
    pub async fn subscribe_to_resource(
        ...
        self.check_permission(plugin_id, McpPermission::Subscribe).await?;
    ...
    pub async fn list_prompts(&self, plugin_id: &str, server_name: Option<&str>) -> Result<Vec<PromptInfo>, McpError> {
        self.check_permission(plugin_id, McpPermission::List).await?;
        
        let state = self.state.read().await;
        
        let prompts = if let Some(server) = server_name {
            state.prompts.get(server)
                .cloned()
                .unwrap_or_default()
        } else {
    ...
    pub async fn set_rate_limit(&self, plugin_id: &str, max_requests: usize, window: Duration) -> Result<(), McpError> {
        let mut state = self.state.write().await;
    ...
    pub async fn register_mock_server(&self, name: &str, config: MockServerConfig) {
        ...
        // Register server
        state.servers.insert(name.to_string(), ServerInfo {
            name: name.to_string(),
            is_active: true,
    ...
    pub async fn set_mock_timeout(&self, server_name: &str, should_timeout: bool) {
        let mut state = self.state.write().await;
        state.mock_timeouts.insert(server_name.to_string(), should_timeout);
    }
    
    #[cfg(test)]
    pub async fn set_mock_invalid_response(&self, server_name: &str, should_fail: bool) {
        let mut state = self.state.write().await;
        state.mock_invalid_responses.insert(server_name.to_string(), should_fail);
    }
    
    #[cfg(test)]
    pub async fn grant_tool_permission(&self, plugin_id: &str, server_name: &str, tools: Vec<&str>) {
        let mut state = self.state.write().await;
        let server_perms = state.tool_permissions.entry(plugin_id.to_string()).or_insert_with(HashMap::new);
        server_perms.insert(server_name.to_string(), tools.iter().map(|t| t.to_string()).collect());
        
        // Also grant invoke permission
        self.grant_permission(plugin_id, McpPermission::Invoke).await;
    }
    
    #[cfg(test)]
    pub async fn grant_server_permission(&self, plugin_id: &str, server_name: &str) {
        let mut state = self.state.write().await;
        let servers = state.server_permissions.entry(plugin_id.to_string()).or_insert_with(Vec::new);
    ...
    #[cfg(test)]
    pub async fn emit_resource_update_internal(&self, server_name: &str, uri: &str, update: ResourceUpdate) {
        let state = self.state.read().await;
        let key = format!("{}:{}", server_name, uri);
