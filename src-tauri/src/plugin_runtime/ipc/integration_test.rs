...
mod tests {
    ...
    async fn test_full_ipc_flow() {
        ...
        // Grant vault read permission for testing
        {
            ...
            permission_manager.write().await.grant_permissions(
                "test-plugin",
                vec![permission]
            ).await.unwrap();
        }
        ...
        let result = handler.handle_api_call("test-plugin", "vault.list", params).await;
    ...
    async fn test_ipc_message_routing() {
        ...
        bridge.register_handler("test.method", |params| {
    ...
    async fn test_concurrent_ipc_calls() {
        ...
        let task1 = tokio::spawn(async move {
            ...
            handler1.handle_api_call("plugin-1", "vault.list", params).await
        });
        
        let task2 = tokio::spawn(async move {
            let params = json!({ "path": "" });
            handler2.handle_api_call("plugin-2", "vault.list", params).await
        });
