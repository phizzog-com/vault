...
use super::{IpcMessage, IpcError};
use crate::plugin_runtime::apis::{ApiManager, VaultError, WorkspaceError, SettingsError};
...
impl PluginApiHandler {
    ...
    pub async fn handle_api_call(
        ...
        match api_name {
            ...
            "workspace" => self.handle_workspace_api(plugin_id, &api_method, params).await,
            "settings" => self.handle_settings_api(plugin_id, &api_method, params).await,
            _ => Err(IpcError {
                code: -32601,
    ...
    async fn handle_vault_api(
        ...
        match method {
            ...
            "deleteFolder" => {
                ...
                match api_manager.vault.delete_folder(plugin_id, path, recursive).await {
                    Ok(()) => Ok(json!({ "success": true })),
                    Err(e) => Err(self.vault_error_to_ipc_error(e)),
                }
            },
    ...
    async fn handle_workspace_api(
        ...
        match method {
            "showNotice" => {
                ...
                let notice_type = params["type"].as_str()
                    .and_then(|t| match t {
                        "info" => Some(crate::plugin_runtime::apis::workspace::NoticeType::Info),
                        "success" => Some(crate::plugin_runtime::apis::workspace::NoticeType::Success),
                        "warning" => Some(crate::plugin_runtime::apis::workspace::NoticeType::Warning),
                ...
                match api_manager.workspace.show_notice(plugin_id, notice_type, message, timeout).await {
                    Ok(()) => Ok(json!({ "success": true })),
                    Err(e) => Err(self.workspace_error_to_ipc_error(e)),
                }
            },
            "getActiveFile" => {
                match api_manager.workspace.get_active_file(plugin_id).await {
            ...
            "openFile" => {
                ...
                match api_manager.workspace.open_file(plugin_id, path, new_pane).await {
                    Ok(()) => Ok(json!({ "success": true })),
                    Err(e) => Err(self.workspace_error_to_ipc_error(e)),
                }
            },
    ...
    async fn handle_settings_api(
        ...
        match method {
            ...
            "getAll" => {
                match api_manager.settings.get_all(plugin_id).await {
...
mod tests {
    ...
    async fn test_handle_vault_read() {
        ...
        let result = handler.handle_api_call("test-plugin", "vault.read", params).await;
    ...
    async fn test_handle_vault_write() {
        ...
        let result = handler.handle_api_call("test-plugin", "vault.write", params).await;
        // Will fail due to permission check, but structure is correct
        assert!(result.is_err());
    }
    ...
    async fn test_handle_workspace_show_notice() {
        ...
        let result = handler.handle_api_call("test-plugin", "workspace.showNotice", params).await;
        // Will fail due to permission check, but structure is correct
        assert!(result.is_err());
    }
    ...
    async fn test_handle_settings_get() {
        ...
        let result = handler.handle_api_call("test-plugin", "settings.get", params).await;
        // Will fail due to permission check, but structure is correct
        assert!(result.is_err());
    }
    ...
    async fn test_invalid_method_format() {
        ...
        let result = handler.handle_api_call("test-plugin", "invalidmethod", json!({})).await;
        assert!(result.is_err());
    ...
    async fn test_unknown_api() {
        ...
        let result = handler.handle_api_call("test-plugin", "unknown.method", json!({})).await;
        assert!(result.is_err());
