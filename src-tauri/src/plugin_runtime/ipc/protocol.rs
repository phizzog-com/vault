...
use super::{IpcMessage, IpcError, IpcBridgeError};
...
impl MessageRouter {
    ...
    pub async fn process_message(&self, message: IpcMessage) -> Result<IpcMessage, IpcBridgeError> {
        match message {
            ...
            IpcMessage::Notification { ref method, ref params } => {
                // Check rate limit
                if !self.check_rate_limit(method).await {
    ...
    async fn check_rate_limit(&self, method: &str) -> bool {
        ...
        let limit = limits.entry(method.to_string())
    ...
    pub async fn set_rate_limit(&self, method: &str, max_requests: u32, window_seconds: u64) {
        let mut limits = self.rate_limits.write().await;
        limits.insert(method.to_string(), RateLimit::new(max_requests, window_seconds));
    }
}
...
impl MessageValidator {
    ...
    pub fn validate_request(id: &str, method: &str, params: &Value) -> Result<(), IpcBridgeError> {
        if id.is_empty() {
            return Err(IpcBridgeError::InvalidMessage("Request ID cannot be empty".to_string()));
        }
        
        if method.is_empty() {
            return Err(IpcBridgeError::InvalidMessage("Method name cannot be empty".to_string()));
        }
        
        // Validate method format (should be namespace.method)
        if !method.contains('.') {
            return Err(IpcBridgeError::InvalidMessage(
                format!("Invalid method format: {}. Expected namespace.method", method)
            ));
        }
    ...
    pub fn validate_params(method: &str, params: &Value) -> Result<(), IpcBridgeError> {
        match method {
            ...
            "vault.write" => {
                if !params.is_object() || !params.get("path").is_some() || !params.get("content").is_some() {
                    return Err(IpcBridgeError::InvalidMessage(
...
mod tests {
    ...
    async fn test_message_router() {
        ...
        router.register_handler("test.echo", |params| {
            Ok(params)
        });
    ...
    fn test_param_validation() {
        ...
        let result = MessageValidator::validate_params("vault.write", &json!({
            "path": "/test",
            "content": "data"
        }));
        assert!(result.is_ok());
