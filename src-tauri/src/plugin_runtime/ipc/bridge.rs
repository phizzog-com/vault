...
impl Bridge {
    ...
    pub async fn queue_message(&self, plugin_id: String, message: IpcMessage) -> Result<(), IpcBridgeError> {
        // Check message size
...
mod tests {
    ...
    async fn test_message_queueing() {
        ...
        let result = bridge.queue_message("plugin1".to_string(), message.clone()).await;
        assert!(result.is_ok());
    ...
    fn test_create_response() {
        let response = Bridge::create_response(
            "req1".to_string(),
