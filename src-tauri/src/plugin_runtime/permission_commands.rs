...
use super::permissions::{PermissionManager, Capability, Permission, ConsentRequest, ConsentResponse};
use std::sync::Arc;
...
pub async fn plugin_request_permission_v2(
    ...
    let parsed_capability = parse_capability_string(&capability)
    ...
    manager.simulate_user_consent_response(&consent_request, response)
        .await
...
pub async fn plugin_get_permissions(
    ...
    Ok(permissions.into_iter().map(|p| PermissionInfo {
...
pub async fn plugin_revoke_permission(
    ...
    let manager = permission_manager.read().await;
    manager.revoke_permissions(&plugin_id, vec![parsed_capability])
        .await
...
fn parse_capability_string(capability: &str) -> Result<Capability, String> {
    match capability {
        "vault:read" => Ok(Capability::VaultRead { paths: vec!["*".to_string()] }),
        "vault:write" => Ok(Capability::VaultWrite { paths: vec!["*".to_string()] }),
        "vault:delete" => Ok(Capability::VaultDelete { paths: vec!["*".to_string()] }),
        ...
        "settings:read" => Ok(Capability::SettingsRead { keys: vec!["*".to_string()] }),
        "settings:write" => Ok(Capability::SettingsWrite { keys: vec!["*".to_string()] }),
...
mod tests {
    ...
    fn test_capability_to_string() {
        let cap = Capability::VaultRead { paths: vec!["*".to_string()] };
        assert_eq!(capability_to_string(&cap), "vault:read");
        
        let cap = Capability::NetworkAccess { domains: vec!["https://api.example.com".to_string()] };
        assert_eq!(capability_to_string(&cap), "network:https://api.example.com");
        
        let cap = Capability::VaultWrite { paths: vec!["/Readwise/*".to_string()] };
        assert_eq!(capability_to_string(&cap), "vault:write:/Readwise/*");
    }

    #[test]
    fn test_is_safe_capability() {
        assert!(is_safe_capability(&Capability::VaultRead { paths: vec!["*".to_string()] }));
        assert!(is_safe_capability(&Capability::WorkspaceRead));
        assert!(is_safe_capability(&Capability::NotificationShow));
        
        assert!(!is_safe_capability(&Capability::VaultWrite { paths: vec!["*".to_string()] }));
        assert!(!is_safe_capability(&Capability::VaultDelete { paths: vec!["*".to_string()] }));
        assert!(!is_safe_capability(&Capability::NetworkAccess { domains: vec!["*".to_string()] }));
    }
