...
pub enum CommandOutput {
    ...
    Created { path: PathBuf, name: String },
    DevServer { port: u16, url: String },
    Build { output_path: PathBuf, size: u64 },
    Lint { issues: Vec<LintIssue>, passed: bool },
    Test { passed: usize, failed: usize, skipped: usize },
    Validate { valid: bool, issues: Vec<String> },
    Info { manifest: PluginManifest, size: u64 },
}
...
impl VaultPluginCli {
    ...
    async fn create_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        if plugin_path.exists() && !force {
            return Err(CliError::PathExists(plugin_path.to_string_lossy().to_string()));
        }
    ...
    async fn dev_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        if port == 0 {
            port = portpicker::pick_unused_port().ok_or(
                CliError::DevServerError("No available ports".to_string())
            )?;
        }
    ...
    async fn build_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        if source_maps {
            fs::write(dist_path.join("main.js.map"), "{\"version\":3,\"sources\":[]}")?;
        }
                ...
                Ok(json) => {
                    ...
                    if let Ok(manifest) = serde_json::from_value::<PluginManifest>(json.clone()) {
                        ...
                        if manifest.min_api_version.starts_with("99.") {
                            issues.push(format!("API version {} is not supported", manifest.min_api_version));
                        }
                    }
                },
                Err(e) => {
    ...
    async fn info_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        let manifest: PluginManifest = if manifest_path.exists() {
            ...
            return Err(CliError::ValidationFailed("No manifest.json found".to_string()));
        };
    ...
    async fn types_command(&self, args: &[&str]) -> Result<CommandOutput, CliError> {
        ...
        generator.generate_type_package(&api, &types_dir).await
        ...
        generator.generate_vscode_settings(&vscode_dir).await
            .map_err(|e| CliError::CommandFailed(format!("VS Code settings generation failed: {}", e)))?;
        
        // Generate migration guide
        let migration_guide = generator.generate_migration_guide("obsidian").await
            .map_err(|e| CliError::CommandFailed(format!("Migration guide generation failed: {}", e)))?;
        fs::write(project_dir.join("MIGRATION.md"), migration_guide)
            .map_err(CliError::Io)?;
        
        // Generate code snippets
        let snippets = generator.generate_snippets().await
            .map_err(|e| CliError::CommandFailed(format!("Snippets generation failed: {}", e)))?;
        let vscode_snippets_dir = project_dir.join(".vscode");
        fs::create_dir_all(&vscode_snippets_dir)
            .map_err(CliError::Io)?;
        fs::write(vscode_snippets_dir.join("vault-plugin.code-snippets"), snippets)
            .map_err(CliError::Io)?;
    ...
    /// Create plugin structure
    fn create_plugin_structure(&self, path: &Path, name: &str, template: &str) -> Result<(), CliError> {
        // Create directories
        ...
        // Create manifest
        let manifest = PluginManifest {
            id: if template == "readwise" { "readwise".to_string() } else { name.to_string() },
            name: if template == "readwise" { "Readwise".to_string() } else { name.to_string() },
