...
impl PermissionManager {
    ...
    pub async fn grant_permissions(
        ...
        let entry = plugin_perms.entry(plugin_id.to_string())
    ...
    pub async fn request_consent(
        ...
        cache.insert(cache_key, ConsentDecision {
    ...
    fn parse_permission_strings(&self, permissions: &[String]) -> Result<Vec<Capability>, PermissionError> {
        let mut capabilities = Vec::new();
        
        for perm_str in permissions {
            let capability = match perm_str.as_str() {
                "vault:read" => Capability::VaultRead { paths: vec!["*".to_string()] },
                "vault:write" => Capability::VaultWrite { paths: vec!["*".to_string()] },
                "vault:delete" => Capability::VaultDelete { paths: vec!["*".to_string()] },
                ...
                "settings:read" => Capability::SettingsRead { keys: vec!["*".to_string()] },
                "settings:write" => Capability::SettingsWrite { keys: vec!["*".to_string()] },
    ...
    pub async fn persist_permissions(&self, plugin_id: &str) -> Result<(), PermissionError> {
        ...
        fs::create_dir_all(&permissions_dir)
            .map_err(|e| PermissionError::Denied(format!("Failed to create permissions directory: {}", e)))?;
        ...
        let json = serde_json::to_string_pretty(&permissions)
            .map_err(|e| PermissionError::Denied(format!("Failed to serialize permissions: {}", e)))?;
    ...
    pub async fn load_permissions(&self, plugin_id: &str) -> Result<(), PermissionError> {
        ...
        let permissions: Vec<Permission> = serde_json::from_str(&json)
            .map_err(|e| PermissionError::Denied(format!("Failed to deserialize permissions: {}", e)))?;
    ...
    pub async fn generate_csp_for_plugin(&self, plugin_id: &str) -> Result<String, PermissionError> {
    ...
    pub async fn check_path_permission(
        ...
        for perm in permissions {
            ...
            match &perm.capability {
                Capability::VaultRead { paths } if matches!(permission_type, VaultPermission::Read) => {
                    if self.path_matches_patterns(path, paths) {
                        return Ok(());
                    }
                },
                Capability::VaultWrite { paths } if matches!(permission_type, VaultPermission::Write) => {
                    if self.path_matches_patterns(path, paths) {
                        return Ok(());
                    }
                },
                Capability::VaultDelete { paths } if matches!(permission_type, VaultPermission::Delete) => {
                    if self.path_matches_patterns(path, paths) {
                        return Ok(());
                    }
                },
    ...
    pub async fn simulate_user_consent_response(
        ...
        match response {
            ...
            ConsentResponse::GrantOnce => {
                ...
                self.grant_permissions(&request.plugin_id, vec![permission]).await
            },
...
mod tests {
    ...
    async fn test_grant_and_revoke_permissions() {
        ...
        manager.grant_permissions("test_plugin", permissions).await.unwrap();
        ...
        manager.revoke_permissions(
            "test_plugin",
            vec![Capability::VaultRead { paths: vec!["*".to_string()] }]
        ).await.unwrap();
    ...
    async fn test_capability_checking() {
        ...
        manager.grant_permissions("test_plugin", vec![permission]).await.unwrap();
        
        // Check has capability
        let has_cap = manager.has_capability("test_plugin", &Capability::WorkspaceWrite).await;
        assert!(has_cap);
        
        let has_cap = manager.has_capability("test_plugin", &Capability::WorkspaceCreate).await;
        assert!(!has_cap);
    }
    ...
    async fn test_consent_caching() {
        ...
        let granted = manager.request_consent(
            "test_plugin",
            Capability::VaultRead { paths: vec!["*".to_string()] }
        ).await.unwrap();
        assert!(granted); // VaultRead is allowed in simulation
        
        // Second request should use cache
        let granted2 = manager.request_consent(
            "test_plugin",
            Capability::VaultRead { paths: vec!["*".to_string()] }
        ).await.unwrap();
