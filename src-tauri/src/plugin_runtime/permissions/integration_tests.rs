...
mod integration_tests {
    ...
    async fn test_complete_permission_flow() {
        ...
        let result = handler.handle_api_call(plugin_id, "vault.read", params.clone()).await;
        ...
        manager.simulate_user_consent_response(&consent_request, ConsentResponse::GrantAlways)
            .await
            .unwrap();
        
        // Step 4: Verify permission was granted
        assert!(manager.has_capability(plugin_id, &consent_request.capability).await);
    ...
    async fn test_permission_persistence_integration() {
        ...
        // Create first manager and grant permissions
        {
            ...
            assert!(manager.has_capability(plugin_id, &permission.capability).await);
        }
        ...
        // Create new manager and load permissions
        {
            ...
            assert!(new_manager.has_capability(plugin_id, &Capability::GraphWrite).await);
        }
    }
    ...
    async fn test_granular_path_permissions_with_api() {
        ...
        let denied = manager.check_path_permission(
            plugin_id,
            "/private/secret.md",
            VaultPermission::Read
        ).await;
    ...
    async fn test_network_permission_domain_validation() {
        ...
        assert!(manager.check_network_permission(
            plugin_id,
            "https://api.allowed.com/endpoint"
        ).await.is_ok());
        
        assert!(manager.check_network_permission(
            plugin_id,
            "https://cdn.allowed.com/resource"
        ).await.is_ok());
    ...
    async fn test_consent_flow_responses() {
        ...
        let consent_request = ConsentRequest {
            plugin_id: plugin_id.to_string(),
            plugin_name: "Test Plugin".to_string(),
            capability: Capability::VaultDelete { paths: vec!["*".to_string()] },
            reason: "Need to delete files".to_string(),
            consequences: vec!["Delete any file".to_string()],
        };
        
        manager.simulate_user_consent_response(&consent_request, ConsentResponse::Deny)
            .await
            .unwrap();
        ...
        manager.simulate_user_consent_response(&consent_request, ConsentResponse::GrantOnce)
            .await
            .unwrap();
        
        let permissions = manager.get_plugin_permissions(plugin_id).await;
        let clipboard_perm = permissions.iter()
        ...
        let permissions = manager.get_plugin_permissions(plugin_id).await;
        let notification_perm = permissions.iter()
    ...
    async fn test_multi_api_permission_enforcement() {
        ...
        let result = handler.handle_api_call(plugin_id, "vault.write", vault_params).await;
        assert!(result.is_err());
        
        // Test workspace API without permission
        let workspace_params = json!({ "message": "test", "type": "info" });
        let result = handler.handle_api_call(plugin_id, "workspace.showNotice", workspace_params).await;
        assert!(result.is_err());
        
        // Test settings API without permission
        let settings_params = json!({ "key": "test-key", "value": "test-value" });
        let result = handler.handle_api_call(plugin_id, "settings.set", settings_params).await;
        assert!(result.is_err());
        ...
        let permissions = vec![
            ...
            Permission {
                capability: Capability::SettingsWrite { keys: vec!["*".to_string()] },
                granted: true,
                granted_at: Some(chrono::Utc::now()),
                expires_at: None,
            },
        ...
        manager.grant_permissions(plugin_id, permissions).await.unwrap();
