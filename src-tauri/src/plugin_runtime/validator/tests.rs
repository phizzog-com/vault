...
async fn test_invalid_manifest_missing_fields() {
    ...
    let report = result.unwrap();
    assert!(!report.is_valid);
    assert!(report.errors.iter().any(|e| e.rule == "manifest-required-fields"));
}
...
async fn test_manifest_version_format() {
    ...
    let report = result.unwrap();
    assert!(!report.is_valid);
    assert!(report.errors.iter().any(|e| e.rule == "manifest-version-format"));
}
...
async fn test_manifest_permission_validation() {
    ...
    assert!(report.warnings.iter().any(|w| w.rule == "manifest-invalid-permission"));
}
...
async fn test_security_analysis_function_constructor() {
    ...
    let report = result.unwrap();
    assert!(!report.is_safe);
    assert!(report.violations.iter().any(|v| v.rule == "no-function-constructor"));
}
...
async fn test_api_compatibility_valid() {
    ...
    let result = validator.check_api_compatibility(&plugin_path, "1.0.0").await;
    assert!(result.is_ok());
...
async fn test_api_usage_validation() {
    ...
    assert!(report.deprecated_apis.iter().any(|api| api.name.contains("adapter")));
}
...
async fn test_empty_plugin_directory() {
    ...
    assert!(matches!(result.unwrap_err(), ValidationError::ManifestNotFound));
}
