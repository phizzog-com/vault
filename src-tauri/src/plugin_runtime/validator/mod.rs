...
impl PluginValidator {
    ...
    pub async fn validate_plugin(&self, plugin_path: &Path) -> Result<ValidationReport, ValidationError> {
    ...
    /// Validate manifest
    pub async fn validate_manifest(&self, plugin_path: &Path) -> Result<ManifestReport, ValidationError> {
        ...
        let manifest = if let Ok(parsed) = serde_json::from_value::<PluginManifest>(json_value.clone()) {
            parsed
        } else {
            // Create minimal manifest with available fields
            PluginManifest {
                id: json_value.get("id").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                name: json_value.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                version: json_value.get("version").and_then(|v| v.as_str()).unwrap_or("0.0.0").to_string(),
                description: json_value.get("description").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                author: json_value.get("author").and_then(|v| v.as_str()).unwrap_or("").to_string(),
                min_api_version: json_value.get("minApiVersion").and_then(|v| v.as_str()).unwrap_or("1.0.0").to_string(),
                permissions: json_value.get("permissions")
                    .and_then(|v| v.as_array())
                    .map(|arr| arr.iter().filter_map(|v| v.as_str().map(String::from)).collect())
                    .unwrap_or_default(),
                entry_point: json_value.get("entryPoint").and_then(|v| v.as_str()).unwrap_or("dist/main.js").to_string(),
            }
        };
    ...
    /// Analyze security
    pub async fn analyze_security(&self, plugin_path: &Path) -> Result<SecurityReport, ValidationError> {
            ...
            while let Some(entry) = entries.next_entry().await? {
                ...
                if path.extension().and_then(|e| e.to_str()) == Some("ts") ||
                   path.extension().and_then(|e| e.to_str()) == Some("js") {
                    ...
                    if !self.disabled_rules.contains(&"no-eval".to_string()) && content.contains("eval(") {
                    ...
                    if !self.disabled_rules.contains(&"no-require".to_string()) && content.contains("require(") {
                        violations.push(SecurityViolation {
                    ...
                    // Check for innerHTML
                    if !self.disabled_rules.contains(&"no-inner-html".to_string()) && content.contains(".innerHTML") {
                        violations.push(SecurityViolation {
                    ...
                    if !self.disabled_rules.contains(&"no-function-constructor".to_string()) && content.contains("new Function(") {
                        violations.push(SecurityViolation {
    ...
    /// Scan dependencies
    pub async fn scan_dependencies(&self, plugin_path: &Path) -> Result<DependencyReport, ValidationError> {
    ...
    pub async fn check_api_compatibility(&self, plugin_path: &Path, current_version: &str) -> Result<CompatibilityReport, ValidationError> {
        let manifest_path = plugin_path.join("manifest.json");
    ...
    /// Validate API usage
    pub async fn validate_api_usage(&self, plugin_path: &Path) -> Result<ApiUsageReport, ValidationError> {
    ...
    /// Analyze performance
    pub async fn analyze_performance(&self, plugin_path: &Path) -> Result<PerformanceReport, ValidationError> {
    ...
    pub async fn generate_report(&self, report: &ValidationReport) -> String {
        ...
        output.push_str(&format!("Status: {}\n", if report.overall_valid { "PASSED" } else { "FAILED" }));
    ...
    pub async fn generate_json_report(&self, report: &ValidationReport) -> Result<serde_json::Value, ValidationError> {
