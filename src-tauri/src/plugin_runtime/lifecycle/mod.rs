...
impl LifecycleManager {
    ...
    pub async fn validate_manifest(&self, manifest_path: &str) -> Result<PluginManifest, LifecycleError> {
        ...
        if manifest.name.is_empty() {
            return Err(LifecycleError::InvalidManifest("Name is required".to_string()));
        }
        
        if manifest.version.is_empty() {
            return Err(LifecycleError::InvalidManifest("Version is required".to_string()));
        }
        
        if manifest.entry_point.is_empty() {
            return Err(LifecycleError::InvalidManifest("Entry point is required".to_string()));
        }
    ...
    pub async fn install_plugin(&self, source_dir: &Path) -> Result<String, LifecycleError> {
        ...
        let manifest = self.validate_manifest(manifest_path.to_str().unwrap()).await?;
        ...
        // Copy plugin files
        self.copy_plugin_files(source_dir, &plugin_install_dir).await?;
        ...
        states.insert(plugin_id.clone(), PluginState {
    ...
    pub async fn activate_plugin(&self, plugin_id: &str) -> Result<(), LifecycleError> {
        ...
        let state = states.get_mut(plugin_id)
        ...
        manifests.iter()
            .filter_map(|(id, manifest)| {
                states.get(id).map(|state| {
                    (id.clone(), manifest.clone(), state.clone())
                })
            })
            .collect()
    }
...
mod tests {
    ...
    async fn test_manifest_parsing_runtime_format() {
        ...
        assert_eq!(manifest.permissions, vec!["vault:read", "vault:write", "workspace:read"]);
    }
    ...
    async fn test_manifest_parsing_readwise_format() {
        ...
        assert_eq!(manifest.permissions, vec!["vault:read", "vault:write", "workspace:read", "network:request"]);
    }
    ...
    async fn test_manifest_with_metadata() {
        ...
        assert_eq!(manifest.metadata.homepage, Some("https://example.com".to_string()));
    ...
    async fn test_plugin_state_management() {
        ...
        states.insert("test_plugin".to_string(), PluginState {
