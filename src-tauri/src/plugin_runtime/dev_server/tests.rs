...
async fn test_hot_reload() {
    ...
    let reload_event = tokio::time::timeout(
...
async fn test_mock_vault_api() {
    ...
    let mock_result = server.call_mock_api("vault.readFile", serde_json::json!({
        "path": "test.md"
    })).await;
...
async fn test_mock_workspace_api() {
    ...
    // Test workspace API
    let result = server.call_mock_api("workspace.showNotice", serde_json::json!({
        "message": "Test notice"
    })).await;
    ...
    let active_file = server.call_mock_api("workspace.getActiveFile", serde_json::json!({})).await;
...
async fn test_resource_limits() {
    ...
    // Set resource limits
    server.set_resource_limits(ResourceLimits {
        max_memory_mb: 100,
        max_cpu_percent: 50,
...
async fn test_invalid_plugin_path() {
    ...
    assert!(matches!(result.unwrap_err(), DevServerError::PluginNotFound(_)));
}
...
async fn test_full_development_workflow() {
    ...
    // Test mock API
    let api_result = server.call_mock_api("vault.readFile", serde_json::json!({
        "path": "test.md"
    })).await;
...
async fn test_concurrent_api_calls() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            server.call_mock_api("vault.readFile", serde_json::json!({
