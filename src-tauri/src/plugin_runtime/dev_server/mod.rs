...
impl DevServer {
    ...
    pub async fn start(&mut self, config: DevServerConfig) -> Result<ServerInfo, DevServerError> {
        ...
        *self.status.write().await = ServerStatus::Starting;
        ...
        let port = if config.port == 0 {
            portpicker::pick_unused_port()
                .ok_or(DevServerError::PortInUse(0))?
        } else {
        ...
        *self.config.write().await = Some(config.clone());
        ...
        // Initial compilation
        self.compile_typescript(&config.plugin_path, config.source_maps).await?;
    ...
    /// Call mock API
    pub async fn call_mock_api(&self, method: &str, params: serde_json::Value) -> Result<serde_json::Value, DevServerError> {
    ...
    pub async fn check_permission(&self, permission: &str) -> bool {
        self.get_mock_permissions().await.contains(&permission.to_string())
    }
    ...
    fn initialize_mock_apis(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            // Vault API mocks
            apis.insert("vault.readFile".to_string(), Arc::new(|params| {
            ...
            apis.insert("vault.writeFile".to_string(), Arc::new(|params| {
                let path = params.get("path").and_then(|p| p.as_str()).unwrap_or("");
                serde_json::json!({
                    "success": true,
                    "path": path
            ...
            // Workspace API mocks
            apis.insert("workspace.getActiveFile".to_string(), Arc::new(|_| {
                serde_json::json!({
            ...
            apis.insert("workspace.showNotice".to_string(), Arc::new(|params| {
            ...
            // Settings API mocks
            apis.insert("settings.get".to_string(), Arc::new(|params| {
            ...
            apis.insert("settings.set".to_string(), Arc::new(|params| {
                let key = params.get("key").and_then(|k| k.as_str()).unwrap_or("");
                serde_json::json!({
                    "success": true,
                    "key": key
    ...
    /// Compile TypeScript
    async fn compile_typescript(&self, plugin_path: &Path, source_maps: bool) -> Result<(), DevServerError> {
        *self.status.write().await = ServerStatus::Compiling;
        ...
        if !tsconfig_path.exists() {
            ...
            fs::write(&tsconfig_path, serde_json::to_string_pretty(&default_tsconfig)?).await?;
        }
        ...
        if src_path.exists() {
            ...
            while let Some(entry) = entries.next_entry().await? {
                ...
                if path.extension().and_then(|e| e.to_str()) == Some("ts") {
                    ...
                    if content.contains("let x: string = 123") {
                        self.compilation_errors.write().await.push(CompilationError {
            ...
            if elapsed > limits.max_build_time_ms {
                return Err(DevServerError::ResourceLimitExceeded(
                    format!("Build time {} ms exceeds limit {} ms", elapsed, limits.max_build_time_ms)
                ));
            }
        }
    ...
    /// Start HTTP server
    async fn start_http_server(&self, port: u16, config: &DevServerConfig) -> Result<ServerInfo, DevServerError> {
        ...
        let api = warp::path("api")
            ...
            .map(|body: serde_json::Value| {
                ...
                let _params = body.get("params").unwrap_or(&serde_json::Value::Null).clone();
