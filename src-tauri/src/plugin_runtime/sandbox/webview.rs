...
use tauri::{AppHandle, Manager, WebviewWindowBuilder, WebviewWindow, http};
use serde_json::json;
use std::sync::Arc;
...
impl PluginWebview {
    ...
    pub fn set_csp(&mut self, policy: &str) -> Result<(), WebviewError> {
        // Validate CSP policy
        if policy.is_empty() {
            return Err(WebviewError::InvalidCsp("CSP policy cannot be empty".to_string()));
        }
    ...
    pub async fn load_entry_point(&mut self, entry_point: &str) -> Result<(), WebviewError> {
        ...
        if entry_point.is_empty() {
            return Err(WebviewError::InvalidEntryPoint("Entry point cannot be empty".to_string()));
        }
    ...
    pub async fn suspend(&mut self) -> Result<(), WebviewError> {
        ...
        if let Some(window) = &self.window {
            ...
            window.eval("window.__PLUGIN_SUSPENDED__ = true;")
    ...
    pub async fn resume(&mut self) -> Result<(), WebviewError> {
        ...
        if let Some(window) = &self.window {
            ...
            window.eval("window.__PLUGIN_SUSPENDED__ = false;")
    ...
    fn create_sandboxed_html(&self, entry_point: &str) -> Result<String, WebviewError> {
        let csp = self.csp_policy.as_ref()
            .map(|p| p.as_str())
