...
impl Sandbox {
    ...
    pub async fn load_plugin(&mut self, entry_point: &str) -> Result<Option<u32>, SandboxError> {
        ...
        let pid = if let Some(ref app_handle) = self.app_handle {
            self.webview.create_window(app_handle)
                .map_err(|e| SandboxError::WebViewError(format!("Failed to create WebView window: {}", e)))?
        } else {
            None
        };

        // Apply CSP policy
        self.webview.set_csp(&self.config.csp_policy)
            .map_err(|e| SandboxError::WebViewError(e.to_string()))?;
        
        // Configure isolation
        self.webview.configure_isolation()
            .map_err(|e| SandboxError::WebViewError(e.to_string()))?;
        
        // Load the plugin entry point
        self.webview.load_entry_point(entry_point).await
            .map_err(|e| SandboxError::WebViewError(e.to_string()))?;
...
impl SandboxManager {
    ...
    pub async fn create_sandbox(&self, plugin_id: &str) -> Result<Arc<RwLock<Sandbox>>, SandboxError> {
