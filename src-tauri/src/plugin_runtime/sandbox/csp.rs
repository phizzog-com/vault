...
impl CspBuilder {
    ...
    pub fn allow_inline_styles(&mut self) -> &mut Self {
        self.directives.entry("style-src".to_string())
            .and_modify(|v| {
    ...
    pub fn allow_data_images(&mut self) -> &mut Self {
        self.directives.entry("img-src".to_string())
            .and_modify(|v| {
    ...
    pub fn allow_network(&mut self, domains: Vec<String>) -> &mut Self {
        self.directives.entry("connect-src".to_string())
            .and_modify(|v| {
    ...
    pub fn allow_workers(&mut self) -> &mut Self {
        self.directives.insert("worker-src".to_string(), vec!["'self'".to_string()]);
        self
    }

    /// Allow WebAssembly execution
    pub fn allow_wasm(&mut self) -> &mut Self {
        self.directives.entry("script-src".to_string())
            .and_modify(|v| {
...
fn is_valid_directive(directive: &str) -> bool {
    matches!(directive,
        ...
        "style-src-elem" | "style-src-attr" | "upgrade-insecure-requests" |
        "block-all-mixed-content" | "plugin-types" | "sandbox" | "report-uri" |
        "report-to" | "require-sri-for" | "trusted-types" | "navigate-to"
    )
}
