...
pub use plugin_metrics::{PluginMetrics, PluginMetricEvent};
...
impl ResourceMonitor {
    ...
    pub async fn update_usage(&self, plugin_id: &str) -> Result<(), ResourceError> {
        ...
        if should_terminate {
            ...
            metrics.record(plugin_id, PluginMetricEvent::ProcessTerminated {
        ...
        if enforcer.should_warn(plugin_id, &usage) {
            let mut metrics = self.plugin_metrics.lock().await;
            metrics.record(plugin_id, PluginMetricEvent::PerformanceWarning {
        ...
        let mut metrics = self.plugin_metrics.lock().await;
        metrics.record(plugin_id, PluginMetricEvent::ResourceUsage {
    ...
    pub async fn register_webview_process(&self, plugin_id: &str, pid: u32) {
        ...
        let mut metrics = self.plugin_metrics.lock().await;
        metrics.record(plugin_id, PluginMetricEvent::ProcessStarted {
            pid,
            plugin_id: plugin_id.to_string(),
        });
    }
