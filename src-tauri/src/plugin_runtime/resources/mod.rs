...
pub use system_monitor::{SystemMonitor, PluginMetrics};
pub use network_tracker::{NetworkTracker, NetworkStats, NetworkInterceptor};
pub use telemetry::{Telemetry, TelemetryEvent, TelemetryEntry, TelemetrySummary};
pub use enforcer::{ResourceEnforcer, ViolationStats};
...
impl ResourceMonitor {
    pub fn new() -> Self {
        Self {
            ...
            telemetry: Arc::new(tokio::sync::Mutex::new(Telemetry::new())),
            enforcer: Arc::new(tokio::sync::Mutex::new(ResourceEnforcer::new())),
        }
    }

    /// Start monitoring resources for a plugin with a process ID
    pub async fn start_monitoring(&self, plugin_id: &str, pid: Option<u32>) -> Result<(), ResourceError> {
        ...
        let limit = limits.entry(plugin_id.to_string())
    ...
    pub async fn update_usage(&self, plugin_id: &str) -> Result<(), ResourceError> {
        ...
        if should_terminate {
            // Record termination in telemetry
            let mut telemetry = self.telemetry.lock().await;
            telemetry.record(plugin_id, TelemetryEvent::ProcessTerminated {
        ...
        if enforcer.should_warn(plugin_id, &usage) {
            let mut telemetry = self.telemetry.lock().await;
            telemetry.record(plugin_id, TelemetryEvent::PerformanceWarning {
    ...
    pub async fn set_limit(&self, plugin_id: &str, limit: ResourceLimit) -> Result<(), ResourceError> {
    ...
    pub async fn track_network_request(&self, plugin_id: &str, bytes_sent: u64, bytes_received: u64) {
    ...
    pub fn start_monitoring_periodic(&self, _plugin_id: &str, _interval: std::time::Duration) -> tokio::task::JoinHandle<()> {
    ...
    pub async fn register_webview_process(&self, plugin_id: &str, pid: u32) {
        ...
        let mut telemetry = self.telemetry.lock().await;
        telemetry.record(plugin_id, TelemetryEvent::ProcessStarted {
            pid,
            plugin_id: plugin_id.to_string(),
        });
    }
    ...
    pub async fn discover_webview_processes(&self) -> Result<Vec<usize>, ResourceError> {
        self.system_monitor.discover_webview_processes()
            .map(|pids| pids.into_iter().map(|pid| pid.as_u32() as usize).collect())
            .map_err(|e| ResourceError::TerminationFailed(e))
    }
    
    /// Record a telemetry event
    pub async fn record_telemetry(&self, plugin_id: &str, event: TelemetryEvent) {
        let mut telemetry = self.telemetry.lock().await;
        telemetry.record(plugin_id, event);
    }
}
