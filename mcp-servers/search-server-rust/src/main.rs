...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vault_path = env::current_dir()
    ...
    if debug_enabled {
        ...
        if env::var("VAULT_PATH").is_ok() {
            eprintln!("[Rust Search Server] WARNING: VAULT_PATH env var is set but being ignored - using CWD instead");
        }
    }
