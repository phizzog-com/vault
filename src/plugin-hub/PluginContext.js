...
export class PluginContext {
  ...
  async enablePlugin(pluginId) {
    try {
      telemetry.startTimer(`enable_plugin_${pluginId}`);
      await this.api.enablePlugin(pluginId);
      this.updatePluginState(pluginId, { enabled: true });
      telemetry.endTimer(`enable_plugin_${pluginId}`);
      telemetry.trackPluginToggle(pluginId, true);
    } catch (error) {
      console.error('Failed to enable plugin:', error);
      telemetry.trackError(error, { action: 'enable_plugin', pluginId });
  ...
  async disablePlugin(pluginId) {
    try {
      telemetry.startTimer(`disable_plugin_${pluginId}`);
      await this.api.disablePlugin(pluginId);
      this.updatePluginState(pluginId, { enabled: false });
      telemetry.endTimer(`disable_plugin_${pluginId}`);
      telemetry.trackPluginToggle(pluginId, false);
    } catch (error) {
      console.error('Failed to disable plugin:', error);
      telemetry.trackError(error, { action: 'disable_plugin', pluginId });
  ...
  async installPlugin(pluginId, options = {}) {
    try {
      ...
      telemetry.startTimer(`install_plugin_${pluginId}`);
      ...
      telemetry.endTimer(`install_plugin_${pluginId}`);
      telemetry.trackPluginInstall(pluginId, options.source || 'marketplace');
      this.showToast('Plugin installed successfully', 'success');
      return plugin;
    } catch (error) {
      console.error('Failed to install plugin:', error);
      telemetry.trackError(error, { action: 'install_plugin', pluginId });
  ...
  async uninstallPlugin(pluginId) {
    try {
      telemetry.startTimer(`uninstall_plugin_${pluginId}`);
      ...
      telemetry.endTimer(`uninstall_plugin_${pluginId}`);
      telemetry.trackPluginUninstall(pluginId);
      this.showToast('Plugin uninstalled', 'info');
    } catch (error) {
      console.error('Failed to uninstall plugin:', error);
      telemetry.trackError(error, { action: 'uninstall_plugin', pluginId });
  ...
  setSearchQuery(query) {
    ...
    if (query) {
      telemetry.trackSearch(query, this.getFilteredPlugins().length);
    }
  }
