...
async function updateUIWithVault(vaultInfo) {
  ...
  // Auto-start MCP servers when vault opens
  if (window.mcpManager) {
    console.log('🚀 Vault opened, starting MCP servers with path:', vaultInfo.path);
    ...
    const mcpServers = {
      'gaimplan-filesystem-rust': {
        ...
        transport: {
          ...
          args: ['--line-transport'],
          env: {}, // No VAULT_PATH - server will use working_dir
          working_dir: vaultInfo.path // SET WORKING DIRECTORY INSTEAD
        },
        capabilities: {
          tools: true,
          resources: true,
    ...
    // Start or restart each server
    (async () => {
      for (const [serverId, config] of Object.entries(mcpServers)) {
        try {
          // Check if already connected
          const existingClient = window.mcpManager.clients.get(serverId);
          
          if (existingClient) {
            // Server exists, check if path is different
            const oldPath = existingClient.config?.transport?.env?.VAULT_PATH;
            if (oldPath === vaultInfo.path) {
              console.log(`✅ ${serverId} already using correct vault`);
              continue;
            }
            
            // Disconnect old server
            console.log(`🔄 Restarting ${serverId} with new vault...`);
            await window.mcpManager.disconnectServer(serverId);
            await new Promise(r => setTimeout(r, 500));
          }
          
          // Start server with current vault
          console.log(`🚀 Starting ${serverId} with vault: ${vaultInfo.path}`);
          await window.mcpManager.connectServer(serverId, config);
      ...
      console.log('✅ MCP servers ready for vault:', vaultInfo.path);
    })();
  }
