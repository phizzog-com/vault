...
export async function getBundledServers(vaultPath, bundlePath = null) {
  ...
  return bundledServers.map(server => {
    ...
    // Replace ${BUNDLE_PATH} in command
    ...
    // Replace ${BUNDLE_PATH} and ${VAULT_PATH} in args
    ...
    // Replace ${VAULT_PATH} in env values
    if (config.transport.env) {
      for (const key of Object.keys(config.transport.env)) {
        if (typeof config.transport.env[key] === 'string') {
          config.transport.env[key] = config.transport.env[key]
            .replace(/\$\{BUNDLE_PATH\}/g, bundlePath)
            .replace(/\$\{VAULT_PATH\}/g, vaultPath);
        }
      }
    ...
    // Rust servers use CWD as the vault path
