...
export class MCPManager {
  ...
  async startAllEnabledServers() {
    ...
    try {
      ...
      if (!settings?.servers) {
        console.log('[MCPManager] No servers configured');
        return;
      }
      ...
      for (const [serverId, config] of Object.entries(settings.servers)) {
        if (!config.enabled) {
          console.log(`[MCPManager] Server ${serverId} is disabled, skipping`);
          continue;
        }
        ...
        if (updatedConfig.transport?.type === 'stdio') {
          ...
          console.log(`[MCPManager] Setting working_dir for ${serverId} to: ${vaultPath}`);
          
          // Remove VAULT_PATH for Rust servers
          if (serverId.includes('-rust') && updatedConfig.transport.env) {
            delete updatedConfig.transport.env.VAULT_PATH;
          }
        }
        
        // Update VAULT_PATH for non-Rust servers
        if (updatedConfig.transport?.env && !serverId.includes('-rust')) {
        ...
        try {
          ...
          console.log(`[MCPManager] Server ${serverId} started successfully`);
        } catch (error) {
          console.error(`[MCPManager] Failed to start server ${serverId}:`, error);
        }
      }
